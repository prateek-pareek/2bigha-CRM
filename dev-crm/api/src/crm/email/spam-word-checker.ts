import { htmlToPlainTextBasic } from '../shared/crm-email-compliance.util';
import {
...
/** Analyze subject + HTML/plain body for spam trigger words. */
export function analyzeEmailSpamContent(
  subject: string,
  bodyHtmlOrText: string,
): EmailSpamCheckResult {
  const bodyPlain = htmlToPlainTextBasic(bodyHtmlOrText || '');
  ...
  const structuralFlags = [...subjectScan.structural, ...bodyScan.structural];
  const totalPenalty = subjectScan.penalty + bodyScan.penalty;

  return {
    score: Math.max(0, Math.min(100, 100 - totalPenalty)),
    matches,
    structuralFlags,
    subjectScore: Math.max(0, 100 - subjectScan.penalty),
    bodyScore: Math.max(0, 100 - bodyScan.penalty),
    totalPenalty,
  };
}
