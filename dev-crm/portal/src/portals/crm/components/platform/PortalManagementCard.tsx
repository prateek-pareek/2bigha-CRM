...
export default function PortalManagementCard({
  ...
  return (
    <div className="rounded border border-[var(--border-color)] bg-white shadow-sm overflow-hidden flex flex-col">
      {/* Jira style Header */}
      <div className="px-4 py-3 bg-[var(--surface-dim)] border-b border-[var(--border-color)] flex flex-wrap items-center justify-between gap-3">
        <div>
        ...
        <div className="flex items-center gap-1.5">
          <button
            type="button"
            className="inline-flex items-center gap-1 bg-[var(--surface-dim)] hover:bg-[var(--border-color)] text-[#344563] text-xs font-semibold px-2.5 py-1.5 rounded transition"
            onClick={async () => {
              await Promise.all([
                fetchAssignments(),
                fetchLogs(),
      ...
      {/* Jira Style Tab Bar */}
      <div className="px-4 bg-white border-b border-[var(--border-color)] flex gap-4 overflow-x-auto scrollbar-none">
        <button
      ...
      <div className="p-4 flex-1">
        ...
        {activeTab === "requirements" && (
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">
                📋 Client Upload Requirements
              </h3>

              {/* Requirement Addition Form */}
              {isAdmin && (
                <div className="bg-[var(--surface-dim)] border border-[var(--border-color)] rounded p-3 mb-3 space-y-2">
                  <div className="flex flex-wrap gap-2">
                    <select
                      value={newNeedCategory}
                      onChange={(e) => setNewNeedCategory(e.target.value)}
                      className="h-8 rounded border border-[var(--border-color)] px-2 text-xs bg-white focus:border-[var(--primary-dark)] outline-none"
                    >
                    ...
                    <input
                      ...
                      className="h-8 flex-1 min-w-[200px] rounded border border-[var(--border-color)] px-2 text-xs focus:border-[var(--primary-dark)] outline-none"
                    />
                    <input
                      type="date"
                      value={newNeedDueDate}
                      onChange={(e) => setNewNeedDueDate(e.target.value)}
                      className="h-8 w-[140px] rounded border border-[var(--border-color)] px-2 text-xs focus:border-[var(--primary-dark)] outline-none"
                    />
                    <button
                      type="button"
                      onClick={handleAddNeed}
                      className="h-8 px-3 rounded bg-[var(--primary-dark)] text-white text-xs font-semibold hover:bg-[#0065ff] transition"
                    >
                      Request
                    </button>
                  </div>
                  <input
                    ...
                    className="h-8 w-full rounded border border-[var(--border-color)] px-2 text-xs focus:border-[var(--primary-dark)] outline-none"
                  />
                </div>
              )}
              ...
              {/* Requirements List */}
              {loadingNeeds ? (
                <div className="text-xs text-[var(--text-muted)] py-2">Loading requirements...</div>
              ) : needs.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {needs.map((n) => (
                    <div
                      key={n._id}
                      className="flex flex-wrap items-center justify-between gap-3 bg-[var(--surface-dim)] border border-[var(--border-color)] rounded p-2.5 shadow-sm hover:bg-[var(--surface-dim)] transition"
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <span className="rounded bg-slate-200 px-1 py-0.5 text-[9px] font-bold uppercase text-[var(--text-muted)]">
                            {n.category}
                          </span>
                          <span
                            className={cn(
                        ...
                        {n.satisfiedDocUrl && (
                          <div className="mt-1 text-[11px]">
                            <span className="font-semibold text-[var(--text-muted)]">Doc: </span>
                            <a
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="h-8 px-2.5 rounded bg-[var(--error-light)] text-[var(--error)] hover:bg-[#ffbdad] text-xs font-semibold transition"
                        >
                          Delete
                        </button>
                      </div>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {isAdmin && (
                  <button
                    type="button"
                    onClick={addDoc}
                    className="text-[var(--primary-dark)] text-xs font-bold hover:underline flex items-center gap-0.5"
                  >
              ...
              {docs.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {docs.map((doc, idx) => (
                    <div
                      key={`doc-${idx}`}
                      className="group relative bg-[#ffffff] border border-[var(--border-color)] rounded p-3.5 hover:shadow-sm hover:border-[#b3bac5] transition"
                    >
                        ...
                        <div className="flex items-center gap-2.5 overflow-hidden">
                          <div
                            className={cn(
                              ...
                              doc.type === "admin_provided"
                                ? "bg-[var(--primary-light)] text-[#0747a6]"
                          ...
                          <div className="overflow-hidden min-w-0">
                            <input
                              ...
                              className="block w-full bg-transparent border-none p-0 text-xs font-semibold text-[var(--text-main)] focus:ring-0 outline-none truncate"
                            />
                            <p className="text-[9px] text-[var(--text-muted)] font-bold uppercase tracking-wider mt-0.5">
                              {doc.type === "admin_provided"
                                ? "ADMIN SHARED"
                                : "CLIENT UPLOADED"}
                            </p>
                        ...
                        <div className="flex items-center gap-1">
                          {doc.url && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-1 text-[var(--text-muted)] hover:text-[var(--primary-dark)]"
                            >
                              <Download size={14} />
                            </a>
                          )}
                      ...
                      {/* File URL Input */}
                      <div className="mt-3 pt-2 border-t border-[var(--surface-dim)] flex items-center justify-between gap-2">
                        <div className="flex-1 flex items-center gap-1.5 bg-[var(--surface-dim)] rounded px-2 py-1 text-xs">
                          <LinkIcon size={12} className="text-[#8993a4]" />
                          <input
                            value={doc.url}
                            onChange={(e) => updateDoc(idx, "url", e.target.value)}
                            placeholder="URL Link to document..."
                            className="w-full bg-transparent border-none p-0 text-xs text-[var(--text-muted)] focus:ring-0 outline-none truncate"
                          />
                        </div>
                        <button
                          type="button"
                          onClick={() => removeDoc(idx)}
                          className="p-1 rounded text-[var(--text-muted)] hover:text-red-600 hover:bg-[var(--error-light)] transition"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="text-[var(--primary-dark)] text-xs font-bold hover:underline"
                    >
                      + Add
                    </button>
                  )}
                ...
                {milestones.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    {milestones.map((m, idx) => (
                      <div
                        key={`ms-${idx}`}
                        className="flex items-center gap-1.5 bg-[var(--surface-dim)] border border-[var(--border-color)] rounded p-2"
                      >
                        <input
                          value={m.label}
                          onChange={(e) => updateMilestone(idx, "label", e.target.value)}
                          placeholder="Milestone title..."
                          className="h-8 flex-1 min-w-[120px] rounded border border-[var(--border-color)] px-2 text-xs focus:border-[var(--primary-dark)] outline-none"
                        />
                        <select
                          value={m.status}
                          onChange={(e) => updateMilestone(idx, "status", e.target.value)}
                          className="h-8 rounded border border-[var(--border-color)] px-2 text-xs bg-white cursor-pointer"
                        >
                        ...
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="h-8 w-[50px] rounded border border-[var(--border-color)] px-1 text-xs text-center focus:border-[var(--primary-dark)] outline-none"
                          />
                        ...
                        <button
                          ...
                          className="text-red-500 p-1 hover:bg-[var(--error-light)] rounded"
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
              ...
              <div>
                ...
                {deadlines.length === 0 ? (
                  <div className="text-xs text-[var(--text-muted)] italic">No deadlines set.</div>
                ) : (
                  <div className="space-y-1.5">
                    {deadlines.map((d, idx) => (
                      <div
                      ...
                      >
                        ...
                        <input
                          ...
                          className="h-8 w-[130px] rounded border border-[var(--border-color)] px-2 text-xs focus:border-[var(--primary-dark)] outline-none"
                        />
                        <button
                          type="button"
            ...
            {/* Save Buttons */}
            {isAdmin && (
              <div className="pt-2 border-t border-[var(--surface-dim)] flex items-center gap-2">
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => handleSaveConfig(false)}
                  className="bg-[var(--primary-dark)] hover:bg-[#0065ff] text-white text-xs font-semibold px-3 py-1.5 rounded disabled:opacity-60 transition"
                >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="h-9 w-full rounded border border-[var(--border-color)] px-2.5 text-xs bg-white focus:border-[var(--primary-dark)] outline-none"
                >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full min-h-[90px] rounded border border-[var(--border-color)] p-2.5 text-xs focus:border-[var(--primary-dark)] outline-none"
              />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-[var(--surface-dim)]">
              <div>
                <label className="block text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-1.5">
                  Google Workspace Login
                </label>
                <div className="flex items-center justify-between border border-[var(--border-color)] rounded p-2.5 bg-[var(--surface-dim)]">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-9 flex-1 rounded border border-[var(--border-color)] px-2.5 text-xs focus:border-[var(--primary-dark)] outline-none"
                  />
                  <button
                    type="button"
                    onClick={randomPassword}
                    className="h-9 px-2.5 rounded border border-[var(--border-color)] bg-[var(--surface-dim)] hover:bg-[var(--border-color)] text-[#344563] text-xs font-semibold transition"
                  >
                    Generate
                  </button>
                </div>
            ...
            {isAdmin && (
              <div className="pt-4 border-t border-[var(--surface-dim)] flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => handleSaveConfig(false)}
                ...
                <button
                  ...
                  className="border border-[var(--border-color)] hover:bg-[var(--surface-dim)] text-[#344563] text-xs font-semibold px-3 py-1.5 rounded disabled:opacity-60 transition"
                >
                  Save Scope Only
                </button>
                {portalPasswordEnabled && (
                  <button
                    ...
                    className="border border-red-200 hover:bg-red-50 text-[var(--error)] text-xs font-semibold px-3 py-1.5 rounded disabled:opacity-60 transition"
                  >
        ...
        {activeTab === "access" && (
          <div className="space-y-4">
            {canAssign && (
              <div className="bg-[var(--surface-dim)] border border-[var(--border-color)] rounded p-3 flex flex-wrap items-center gap-2.5">
                <div className="flex-1 min-w-[200px]">
                  <select
                    value={selectedEmployee}
                    onChange={(e) => setSelectedEmployee(e.target.value)}
                    className="h-9 w-full rounded border border-[var(--border-color)] px-2 text-xs bg-white focus:border-[var(--primary-dark)] outline-none"
                  >
                ...
                <select
                  ...
                  className="h-9 rounded border border-[var(--border-color)] px-2 text-xs bg-white focus:border-[var(--primary-dark)] outline-none"
                >
                ...
                <button
                  ...
                  className="h-9 px-3 rounded bg-[var(--primary-dark)] hover:bg-[#0065ff] text-white text-xs font-semibold disabled:opacity-60 transition"
                >
                  Assign Access
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Assigned Table */}
              <div className="border border-[var(--border-color)] rounded p-3 bg-white">
                <h4 className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2.5 flex items-center justify-between">
                  <span>Assigned Employees ({assignments.length})</span>
                  <button
                    type="button"
                    onClick={fetchAssignments}
                    className="text-[10px] text-[var(--primary-dark)] hover:underline"
                  >
                    Refresh
                  </button>
                </h4>
                ...
                {loadingAssignments ? (
                  <div className="text-xs text-[var(--text-muted)]">Loading assignments...</div>
                  ...
                  <div className="space-y-2.5">
                    {assignments.map((a) => (
                      <div
                        ...
                        className="flex items-center justify-between gap-2 border-b border-[var(--surface-dim)] pb-2 last:border-0 last:pb-0"
                      >
                        ...
                        {canAssign && (
                          <button
                            type="button"
                            className="bg-[var(--error-light)] hover:bg-[#ffbdad] text-[var(--error)] text-[10px] font-bold px-2 py-1 rounded transition"
              ...
              <div className="border border-[var(--border-color)] rounded p-3 bg-white">
                ...
                {loadingLogs ? (
                  <div className="text-xs text-[var(--text-muted)]">Loading audit log data...</div>
                ) : logs.length === 0 ? (
                  <div className="text-xs text-[var(--text-muted)] italic">
                  ...
                  <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {logs.map((l) => (
                      <div
                        key={l._id}
                        className="text-xs border-b border-[var(--surface-dim)] pb-1.5 last:border-0 last:pb-0"
                      >
                        <p className="text-[var(--text-main)] font-semibold">
                          {l.employeeName} &middot;{" "}
                          <span className="text-[var(--text-muted)] font-medium">{l.action}</span>
                        </p>
        ...
        {activeTab === "chat" && (
          <div className="space-y-4">
            {/* Feed Updates Panel */}
            <div className="border border-[var(--border-color)] rounded p-3.5 bg-white">
              <h4 className="text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-3">
              ...
              {(isAdmin ||
                ...
                <div className="space-y-2 mb-4 bg-[var(--surface-dim)] border border-[var(--border-color)] rounded p-3">
                  <div className="flex flex-wrap gap-2">
                    <input
                      value={updateTitle}
                      onChange={(e) => setUpdateTitle(e.target.value)}
                      placeholder="Title of update (e.g., Weekly Summary #4)"
                      className="h-9 flex-1 min-w-[200px] rounded border border-[var(--border-color)] px-2.5 text-xs focus:border-[var(--primary-dark)] outline-none"
                    />
                    <select
                      value={updateCadence}
                      onChange={(e) => setUpdateCadence(e.target.value as any)}
                      className="h-9 rounded border border-[var(--border-color)] px-2.5 text-xs bg-white outline-none"
                    >
                    ...
                    <button
                      ...
                      className="h-9 px-3 rounded bg-[var(--primary-dark)] text-white text-xs font-semibold hover:bg-[#0065ff] transition"
                    >
                      Post
                    </button>
                    <button
                      ...
                      className="h-9 px-3 rounded border border-[var(--border-color)] hover:bg-[var(--border-color)] text-[#344563] text-xs font-semibold transition"
                    >
                      Post Daily
                    </button>
                    <button
                    ...
                    <button
                      ...
                      disabled={aiDrafting}
                      className="h-9 px-3 rounded border border-[var(--border-color)] hover:bg-[var(--border-color)] text-[#344563] text-xs font-semibold inline-flex items-center gap-1.5 transition disabled:opacity-50"
                    >
                      {aiDrafting ? (
                  ...
                  <input
                    ...
                    className="h-9 w-full rounded border border-[var(--border-color)] px-2 text-xs focus:border-[var(--primary-dark)] outline-none"
                  />
                  <textarea
                    value={updateBody}
                    onChange={(e) => setUpdateBody(e.target.value)}
                    placeholder="Write body of update message here..."
                    className="w-full min-h-[70px] rounded border border-[var(--border-color)] p-2 text-xs focus:border-[var(--primary-dark)] outline-none"
                  />
                </div>
              )}
              ...
              {/* Updates Feed list */}
              {loadingUpdates ? (
                <div className="text-xs text-[var(--text-muted)] py-1">Loading updates feed...</div>
              ) : updates.length === 0 ? (
                ...
                <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
                  {updates.map((u) => (
                    <div
                      key={u._id}
                      className="rounded border border-[var(--border-color)] p-3 bg-[var(--surface-dim)] relative hover:bg-[var(--surface-dim)] transition"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-xs font-bold text-[var(--text-main)]">
                          {u.title} &middot;{" "}
                          <span className="text-[var(--text-muted)] font-semibold text-[10px] uppercase">
                            {u.cadence}
                          </span>
                        </p>
                        {(isAdmin ||
                          ...
                          <button
                            type="button"
                            className="bg-[var(--error-light)] text-[var(--error)] text-[10px] font-bold px-1.5 py-0.5 rounded hover:bg-[#ffbdad]"
                      ...
                      <p className="mt-1.5 whitespace-pre-wrap text-xs text-[var(--text-muted)] leading-relaxed">
                        {u.body}
                      </p>
...
function PortalChatTab({ dealId }: { dealId: string }) {
  ...
  return (
    <div className="flex flex-col h-[350px] bg-[var(--surface-dim)]">
      <div className="px-3 py-2 bg-[var(--surface-dim)] border-b border-[var(--border-color)] flex items-center justify-between">
        ...
        <button
          ...
          className="text-[10px] text-[var(--primary-dark)] font-bold hover:underline"
        >
      ...
      >
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-xs text-[var(--text-muted)] text-center p-4">
            <span className="font-semibold text-[var(--text-main)]">No conversations yet</span>
                ...
                <div className="space-y-0.5">
                  <div
                    className={cn(
                      ...
                      isAdminMsg
                        ? "bg-[var(--primary-light)] text-[#0747a6] border-[#b3d4ff] rounded-tr-none"
                  ...
                  <div
                    className={cn(
                      "flex items-center gap-1 text-[9px] text-[var(--text-muted)] font-medium px-1",
      ...
      <form
        onSubmit={handleSend}
        className="p-2 bg-white border-t border-[var(--border-color)] flex gap-2"
      >
        <input
          type="text"
          value={inputText}
          ...
          disabled={sending}
          className="flex-1 h-8 px-2.5 border border-[var(--border-color)] rounded text-xs outline-none focus:border-[var(--primary-dark)] focus:ring-0 min-w-0"
        />
        <button
          type="submit"
          disabled={sending || !inputText.trim()}
          className="h-8 px-3 rounded bg-[var(--primary-dark)] hover:bg-[#0065ff] text-white text-xs font-semibold transition"
        >
          Send
        </button>
      </form>
