...
export default function OrganizationAssociationsPanel({
  ...
  const leads = (org?.associatedLeads as AnyRec[]) || [];
  ...
  return (
    <div className="bg-card border border-border rounded-[var(--crm-radius-ui)] p-6 shadow-sm">
      ...
      <p className="text-xs text-text-muted mb-4 leading-relaxed">
        Link leads, contacts, and deals to this company. Their associated activity is included in the company timeline.
      </p>
        ...
        <div className="mb-5 p-4 rounded-[var(--radius-md)] border border-border bg-surface-dim/30 space-y-3">
          ...
          {results && q.trim().length >= 2 && (
            <div className="max-h-56 overflow-y-auto space-y-3 text-sm">
                    ...
                    {results.leads!.map((lead) => (
                      <li key={lead._id}>
                        <button
                          type="button"
                          className="w-full text-left px-2 py-1.5 rounded-lg hover:bg-card border border-transparent hover:border-border"
                          onClick={() => addExisting("associatedLeads", lead._id)}
                        >
                          {contactLabel(lead)}
                        </button>
                      </li>
              ...
              {!searching &&
                q.trim().length >= 2 &&
                !(results.deals?.length || results.contacts?.length || results.leads?.length) && (
            ...
            <div className="space-y-2">
              {leads.map((lead) => (
                <Row
                  ...
                  title={contactLabel(lead)}
                  subtitle={lead.email || lead.stage}
                  onRemove={() => remove("associatedLeads", lead._id)}
                />
              ))}
            </div>
          )}
