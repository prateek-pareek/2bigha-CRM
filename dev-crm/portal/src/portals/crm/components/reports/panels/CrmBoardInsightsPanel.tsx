...
export default function CrmBoardInsightsPanel({
  ...
  const emailPie = useMemo(() => {
    ...
    return {
      ...
      clicks: [
        { name: "Clicked", value: s.clicked, color: CRM_CHART_SECONDARY },
  ...
  return (
    <section
      className={`rounded-[var(--crm-radius-ui)] border border-[var(--border-color)] bg-card/90 shadow-sm overflow-hidden ${className}`}
    >
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <div className="p-2 rounded-[var(--radius-md)] bg-primary/10 text-primary shrink-0">
            <BarChart3 size={20} />
          </div>
        ...
        <div className="px-5 pb-5 space-y-5 border-t border-slate-50">
          ...
          {data && (
            <>
              ...
              <div className="rounded-[var(--radius-md)] border border-[var(--border-color)] bg-surface-dim/20 p-5 space-y-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <CrmDropdown
                    ...
                    options={pipelineOptions.map((opt) => ({ value: opt.key, label: opt.label }))}
                  />
                </div>
              ...
              {emailPie && data?.emailEngagementSummary && (
                <div className="rounded-[var(--radius-md)] border border-[var(--border-color)] bg-surface-dim/20 p-5 space-y-4">
                  <div>
                    <h3 className="text-xs font-semibold text-text-muted">
              ...
              {data.followUpHealth != null && data.engagementByDay != null && (
                <div className="pt-4 border-t border-[var(--border-color)] space-y-4">
                  <div>
                    ...
                    <div className="rounded-[var(--radius-md)] border border-[var(--hs-link)] bg-[var(--hs-link)]/5 p-4 mb-4">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-surface-dim/30 rounded-[var(--radius-md)] border border-[var(--border-color)] p-5 flex flex-col items-center justify-center">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <div className="rounded-[var(--radius-md)] border border-[var(--border-color)] overflow-hidden max-h-[180px] overflow-y-auto custom-scrollbar">
                ...
                <div className="rounded-xl border border-[#e2e8f0] bg-white p-5 shadow-[rgba(219,219,219,0.25)_0px_4px_4px_0px]">
                  ...
                  <div className="mb-3 grid grid-cols-3 gap-2">
                    <div className="rounded-lg bg-blue-50/80 p-2 border border-blue-100">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-blue-700/80">Total</p>
                      <p className="text-base font-extrabold tabular-nums text-blue-700">{leadsAddedStats.total}</p>
                    </div>
                    <div className="rounded-lg bg-[#f8fafc] p-2 border border-[#e2e8f0]">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Avg / day</p>
                      <p className="text-base font-extrabold tabular-nums text-slate-700">{leadsAddedStats.avg}</p>
                    </div>
                    <div className="rounded-lg bg-amber-50/80 p-2 border border-amber-100">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-amber-700/80">Peak</p>
                      <p className="text-xs font-bold text-amber-700 leading-tight">{leadsAddedStats.peakLabel}</p>
                    </div>
                  </div>
                    ...
                    {leadsAddedByDay.length === 0 ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <ComposedChart data={leadsAddedByDay} margin={{ top: 12, right: 8, left: -8, bottom: 0 }}>
                          ...
                          <XAxis dataKey="label" tick={{ fontSize: 10, fill: "#64748b" }} interval="preserveStartEnd" minTickGap={24} />
                          <YAxis tick={{ fontSize: 10, fill: "#64748b" }} allowDecimals={false} domain={[0, maxLeadsDay] as [number, number]} />
                          <Tooltip
                          ...
                          <Bar dataKey="count" name="Leads" radius={[6, 6, 0, 0]} barSize={leadsAddedByDay.length > 20 ? 10 : 16}>
                            {leadsAddedByDay.map((row, i) => (
                              <Cell key={i} fill={row.isPeak ? "#f59e0b" : "#3b82f6"} fillOpacity={row.count === 0 ? 0.2 : 0.9} />
                            ))}
                            <LabelList dataKey="count" position="top" fill="#64748b" fontSize={9} formatter={(v: any) => v || ""} />
                          </Bar>
                ...
                <div className="rounded-xl border border-[#e2e8f0] bg-white p-5 shadow-[rgba(219,219,219,0.25)_0px_4px_4px_0px]">
                  ...
                  <div className="h-[260px] w-full">
                    {funnelByDay.every(
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={funnelByDay} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" className="stroke-slate-100" vertical={false} />
                          ...
                          <Line type="monotone" dataKey="sends" name="Email sends" stroke={CRM_CHART_PRIMARY} strokeWidth={2} dot={false} />
                          <Line type="monotone" dataKey="opens" name="Email opens" stroke="#10b981" strokeWidth={2} dot={false} />
                          <Line type="monotone" dataKey="replies" name="Email replies" stroke={CRM_CHART_SECONDARY} strokeWidth={2} dot={false} />
                          <Line
                            type="monotone"
                            dataKey="touches"
                            name="Touches"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {(data.followUpReplyAnalytics?.repliesByAttempt ?? []).length === 0 ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data.followUpReplyAnalytics?.repliesByAttempt ?? []}>
                          <CartesianGrid strokeDasharray="3 3" className="stroke-slate-100" vertical={false} />
                          <XAxis dataKey="label" tick={{ fontSize: 10, fill: "#64748b" }} interval={0} angle={-12} textAnchor="end" height={45} />
                          <YAxis tick={{ fontSize: 10, fill: "#64748b" }} allowDecimals={false} />
                          <Tooltip />
                          <Bar dataKey="replies" fill={CRM_CHART_SECONDARY} radius={[6, 6, 0, 0]} name="Replies">
                            <LabelList
                              dataKey="replies"
                              position="top"
                              fill="#64748b"
                ...
                <div className="rounded-xl border border-[#e2e8f0] bg-white p-5 shadow-[rgba(219,219,219,0.25)_0px_4px_4px_0px]">
                  ...
                  {(data.channelPerformance ?? []).length > 0 && (
                    <div className="mt-3 overflow-x-auto rounded-[var(--radius-md)] border border-[var(--border-color)]">
                      <table className="w-full text-left text-[11px]">
                        ...
                        <tbody>
                          {(data.channelPerformance ?? []).slice(0, 10).map((row) => (
                            <tr key={row.channel} className="border-t border-[var(--border-color)]">
                ...
                <div className="rounded-xl border border-[#e2e8f0] bg-white p-5 shadow-[rgba(219,219,219,0.25)_0px_4px_4px_0px]">
                  ...
                  <div className="h-[200px] w-full">
                    {data.emailOpensByDay.length === 0 ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data.emailOpensByDay} margin={{ top: 12, right: 8, left: -8, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" className="stroke-slate-100" vertical={false} />
                          <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#64748b" }} />
                          <YAxis tick={{ fontSize: 10, fill: "#64748b" }} allowDecimals={false} domain={[0, maxOpenDay] as [number, number]} />
                          <Tooltip />
                          <Bar dataKey="sendsOpened" fill="#10b981" radius={[6, 6, 0, 0]} name="Sends opened">
                            <LabelList dataKey="sendsOpened" position="top" fill="#10b981" fontSize={10} fontWeight={700} formatter={(v: any) => v || ""} />
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    )}
                ...
                <div className="rounded-xl border border-[#e2e8f0] bg-white p-5 shadow-[rgba(219,219,219,0.25)_0px_4px_4px_0px]">
                  ...
                  <div className="h-[200px] w-full">
                    {(data.emailRepliesByDay ?? []).length === 0 ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data.emailRepliesByDay} margin={{ top: 12, right: 8, left: -8, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" className="stroke-slate-100" vertical={false} />
                          <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#64748b" }} />
                          <YAxis tick={{ fontSize: 10, fill: "#64748b" }} allowDecimals={false} domain={[0, maxReplyDay] as [number, number]} />
                          <Tooltip />
                          <Bar dataKey="repliesReceived" fill={CRM_CHART_SECONDARY} radius={[6, 6, 0, 0]} name="Replies">
                            <LabelList dataKey="repliesReceived" position="top" fill="#64748b" fontSize={10} fontWeight={700} formatter={(v: any) => v || ""} />
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="rounded-[var(--radius-md)] border border-[var(--border-color)] overflow-hidden max-h-[220px] overflow-y-auto custom-scrollbar">
                    <table className="w-full text-xs">
                      <thead className="bg-surface-dim/80 sticky top-0">
                        <tr className="text-left text-text-muted font-black uppercase tracking-tighter">
                ...
                <div>
                  ...
                  <div className="rounded-[var(--radius-md)] border border-[var(--border-color)] overflow-hidden max-h-[280px] overflow-y-auto custom-scrollbar">
                    <table className="w-full text-xs">
                      <thead className="bg-surface-dim/80 sticky top-0">
                        <tr className="text-left text-text-muted font-black uppercase tracking-tighter">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <ComposedChart data={rows} layout="vertical" margin={{ left: 2, right: 16, top: 8, bottom: 4 }}>
                            ...
                            <XAxis
                              ...
                              domain={[0, 100]}
                              tickFormatter={(v) => `${v}%`}
                              tick={{ fontSize: 9, fill: CRM_CHART_SECONDARY }}
                            />
                            ...
                            />
                            <Legend wrapperStyle={{ fontSize: 10 }} />
                            <Bar dataKey="sends" name="Sends" fill={CRM_CHART_PRIMARY} radius={[0, 5, 5, 0]} barSize={10} />
                            ...
                            <Line
                              ...
                              strokeWidth={2}
                              dot={{ r: 3, fill: CRM_CHART_SECONDARY, strokeWidth: 0 }}
                            />
                          </ComposedChart>
                        </ResponsiveContainer>
                      );
...
function StatPill({
  ...
  return (
    <div className="rounded-[var(--radius-md)] border border-[var(--border-color)] bg-surface-dim/40 px-3 py-3 flex items-start gap-2">
