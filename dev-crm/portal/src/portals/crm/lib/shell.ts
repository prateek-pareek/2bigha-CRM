/**
 * CRM app shell tokens — owned by CRM so suite/PM Jira chrome is optional.
 * Layout root should use `crmSuiteShellClassName` + `data-crm-app`.
 * Visual reference: https://crms.dreamstechnologies.com/html/contacts.html
 */

import { suiteSidebarChrome } from "@/components/suite/shell/sidebar-chrome";

/** Root class for `/crm` layout — no dependency on PM jiraSuiteShellClassName */
export const crmSuiteShellClassName =
  "theme-crm-hubspot crm-app-root min-h-screen bg-[var(--background)] text-[var(--text-main)]";

/** Main content area under AppShell when on CRM routes */
...
/**
 * Top app header — CRMS white bar, soft icon chips, red product mark.
 * Used by ModuleJiraAppHeader when productHref starts with `/crm`.
 */
export const crmAppChrome = {
  /** CRMS navbar-header ~57px white bar */
  header:
  ...
  /** CRMS topbar-link — bordered 38×38 */
  iconChip:
  ...
  /** CRMS header-search — left of actions, not full-bleed center */
  search: "crm-header-search me-auto hidden min-w-0 max-w-[280px] flex-1 lg:block xl:max-w-[320px]",
...
/**
 * Sidebar chrome for CRM — global suite rail (soft pill + icon tiles).
 */
export const crmSidebarChrome = {
  ...suiteSidebarChrome,
