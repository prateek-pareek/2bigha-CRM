...
import NotificationList from './hrms/NotificationList';
...
import { PmJiraAppHeader } from '@/components/pm/layout/pm-jira-app-header';
import { ModuleJiraAppHeader } from '@/components/layout/module-jira-app-header';
import { jiraShellLayout } from '@/lib/pm/jira-ui';
import { Building2, Globe, Handshake, KeyRound, LineChart, Share2 } from 'lucide-react';
...
const QuickAddModal = dynamic(() => import('./crm/QuickAddModal'), { ssr: false });
const LazyPMGlobalSearch = dynamic(() => import('@/components/pm/layout/LazyPMGlobalSearch'), { ssr: false });
const LazyGlobalSearch = dynamic(() => import('@/components/LazyGlobalSearch'), { ssr: false });
...
export default function AppShell({ children }: { children: React.ReactNode }) {
    ...
    const toggleSidebarPinned = () => {
        setIsSidebarPinned(prev => {
            const next = !prev;
            // If collapsing, also clear hover so sidebar doesn't stay expanded
    ...
    return (
        <div className="theme-crm-hubspot theme-collab-slack crm-app-shell slack-app-shell pm-jira-font flex h-screen flex-col overflow-hidden">
