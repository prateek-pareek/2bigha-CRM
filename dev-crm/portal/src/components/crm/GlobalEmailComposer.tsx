...
function buildComposerEmailBodyHtml(
  ...
  meta: { fromLabel: string; dateLabel: string } | null,
): string {
  ...
  const quotedBlock = `<blockquote style="margin:20px 0 0 0;padding:12px 0 0 0;border-top:1px solid #eaeaea;border-left:none;">` +
    ...
    `<strong>Original message</strong> · ${when}<br/>` +
    `<span style="color:var(--text-main);">${from}</span></p>` +
...
export default function GlobalEmailComposer() {
  ...
  }, [toInputDraft, isOpen, crmInboxMode]);

  useEffect(() => {
    if (!isOpen || !crmInboxMode) return;
    const onDoc = (e: MouseEvent) => {
      const el = composeSearchWrapRef.current;
                      ...
                      {lockRecipient ? (
                        ...
                        <>
                          ...
                          <input
                            type="text"
                            value={toInputDraft}
                            onChange={(e) => setToInputDraft(e.target.value)}
                            onKeyDown={(e) => {
                      ...
                      <div className="flex gap-3 border-t border-[var(--border-color)] px-3 py-2 sm:items-start">
                        ...
                        <div className="flex min-w-0 flex-1 flex-wrap gap-1.5 py-1">
                          ...
                          <input
                            ...
                            placeholder={ccEmails.length ? "" : "Add Cc"}
                      ...
                      <div className="flex gap-3 border-t border-[var(--border-color)] px-3 py-2 sm:items-start">
                        ...
                        <div className="flex min-w-0 flex-1 flex-wrap gap-1.5 py-1">
                          ...
                          <input
                            ...
                            placeholder={bccEmails.length ? "" : "Add Bcc"}
                            className="h-8 min-w-[140px] flex-1 bg-transparent text-sm outline-none placeholder:text-[var(--text-muted)]"
                          />
                        </div>
                      </div>
              ...
              {replyQuotedHtml?.trim() ? (
                <div className="mt-4 space-y-2">
                  <p className="text-xs font-semibold text-[var(--text-muted)]">
                    Original message (same as received — included when you send)
                  </p>
                  <iframe
