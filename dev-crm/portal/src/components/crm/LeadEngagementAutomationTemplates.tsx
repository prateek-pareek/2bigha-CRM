...
export default function LeadEngagementAutomationTemplates({ canEdit }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {editing && canEdit ? (
        <div className="rounded-[3px] border border-[var(--hs-link)]/30 bg-[#fff8f6] p-5 space-y-4">
          ...
          <div className="rounded-lg border border-[var(--border-color)] bg-white p-4 space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {canEdit ? (
                <button
                  type="button"
                  className="inline-flex items-center gap-1 rounded-md border border-[var(--border-color)] bg-white px-2.5 py-1.5 text-xs font-semibold text-[var(--text-main)] hover:bg-[#f4f5f7]"
                  onClick={() =>
                    setEditRules({
                      ...editRules,
                      stageRules: [
            ...
            {(editRules.stageRules || []).length === 0 ? (
              ...
              <div className="space-y-3">
                {(editRules.stageRules || []).map((rule, index) => (
                  <div
                    key={rule.id}
                    className="rounded-lg border border-[#dfe1e6] bg-[#f4f5f7]/60 p-3 space-y-2"
                  >
