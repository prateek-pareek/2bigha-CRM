...
interface CRMFilterBarProps {
  module: 'leads' | 'contacts' | 'organizations' | 'deals' | 'activities' | 'inbox' | 'clients' | 'platform-opportunities';
...
export default function CRMFilterBar({ module, filters, onChange, onClear, onPropertiesReady, pipelineId }: CRMFilterBarProps) {
  ...
  useEffect(() => {
    ...
    fetch(`${CRM_API_URL}/custom-fields?module=${module}`, {
      ...
      .then((customFields: any[]) => {
        const cfProps = (customFields || []).map(customFieldToFilterProperty);
        const merged = [...staticProps, ...cfProps];
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <>
          <div className="absolute top-full left-0 mt-2 z-50 w-[420px] max-w-[95vw] bg-card rounded-[3px] border border-[#ebecf0] shadow-xl overflow-hidden">
            <div className="p-4 border-b border-[#ebecf0] bg-surface-dim/50 flex justify-between items-center">
              <span className="text-xs font-bold text-text-muted">Filter by property</span>
              <div className="flex gap-2">
                ...
                <button
                  onClick={addFilter}
                  className="flex items-center gap-1.5 text-xs font-bold text-primary hover:text-primary-dark uppercase"
                >
                  <Plus size={12} />
                  Add filter
                </button>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2 flex-wrap">
                      <select
                        ...
                        className="flex-1 min-w-[140px] px-3 py-2 bg-surface-dim border border-[#dfe1e6] rounded-[3px] text-sm font-medium text-text-main"
                      >
                        {properties.map(pr => (
                      ...
                      <select
                        ...
                        className="w-[140px] px-3 py-2 bg-surface-dim border border-[#dfe1e6] rounded-[3px] text-sm font-medium text-text-main"
                      >
                        {ops.map(o => (
                      ...
                      {showValue && (
                        p?.type === 'select' && p.options?.length ? (
                          <select
                            ...
                            className="flex-1 min-w-[120px] px-3 py-2 bg-surface-dim border border-[#dfe1e6] rounded-[3px] text-sm font-medium text-text-main"
                          >
                            ...
                            <div className="flex gap-1 overflow-x-auto no-scrollbar pb-1">
                              {[
                                ...
                                <button
                                  ...
                                  className="px-2 py-1 bg-white border border-[#dfe1e6] rounded-lg text-[9px] font-black uppercase text-text-muted hover:text-primary hover:border-primary/30 transition-all shrink-0"
                                >
                                  {quick.label}
                                </button>
                              ))}
                            ...
                            {f.operator === 'between' ? (
                              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                <DatePickerField
                                  ...
                                  buttonClassName="h-10 w-full justify-start rounded-[3px] border-[#dfe1e6] bg-surface-dim px-3 text-xs font-semibold"
                                />
                              ...
                              <DatePickerField
                                ...
                                disableFuture
                                buttonClassName="h-10 w-full justify-start rounded-[3px] border-[#dfe1e6] bg-surface-dim px-3 text-sm font-medium"
                              />
                            )}
                          </div>
                        ) : p?.type === 'checkbox' ? (
