...
import type { WorkflowCanvasEntityKind } from "@/components/crm/WorkflowCanvasEditor";
import {
...
} from "@/lib/crm/workflow-filter-utils";
...
const WorkflowCanvasEditor = dynamic(
  () => import("@/components/crm/WorkflowCanvasEditor"),
  {
    ssr: false,
    loading: () => (
      <div className="flex h-[min(50vh,420px)] min-h-[200px] items-center justify-center rounded-[3px] border border-violet-200/60 bg-white/90">
  ...
  return (
    <div ref={ref} className={`relative ${className ?? ""}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between gap-2 rounded-[3px] border border-border/60 bg-surface-dim/30 px-4 py-2.5 text-sm font-medium text-text-main outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all cursor-pointer hover:border-primary/30 hover:bg-primary/5"
      >
      ...
      {open && (
        <ul className="absolute z-50 mt-1 w-full max-h-64 overflow-y-auto rounded-[3px] border border-border bg-card shadow-lg py-1">
          {options.map((opt) => (
            <li key={opt.value}>
              <button
                type="button"
...
export function WorkflowEditorContent({ workflowId }: { workflowId: string }) {
  ...
  const fieldClass =
    "w-full rounded-[3px] border border-border/60 bg-surface-dim/30 px-4 py-2.5 text-sm font-medium text-text-main outline-none placeholder:text-text-muted/60 focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all";
  const selectClass =
    "w-full appearance-none rounded-[3px] border border-border/60 bg-surface-dim/30 px-4 py-2.5 pr-9 text-sm font-medium text-text-main outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all cursor-pointer";
    ...
    <div className="animate-in fade-in duration-300 space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-border pb-4 flex-wrap gap-y-2">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[3px] bg-primary/10 text-primary ring-1 ring-primary/15">
            <GitBranch size={20} />
          </span>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-[3px] bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-md shadow-primary/20 transition-colors hover:bg-primary-dark disabled:pointer-events-none disabled:opacity-40"
          >
      ...
      {isNew && (
        <div className="rounded-[3px] border border-border/60 bg-surface-dim/30 px-4 py-3">
      ...
      <div className="space-y-4">
          <section className="rounded-[3px] border border-border bg-card p-5 space-y-4 shadow-sm ring-1 ring-border/40">
            <h3 className="flex items-center gap-2 border-b border-border pb-3 text-sm font-semibold text-text-main">
              <span className="flex h-9 w-9 items-center justify-center rounded-[3px] bg-primary/10 text-primary ring-1 ring-primary/15">
                <Sparkles size={18} />
              </span>
              Workflow details
            </h3>
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div className="sm:col-span-1 space-y-1">
                ...
                <div className="max-h-[132px] overflow-y-auto rounded-[3px] border border-border/60 bg-surface-dim/30 p-1.5">
                ...
                {form.triggers.filter((x) => x !== form.trigger).length > 0 ? (
                  <div className="mt-1.5 space-y-1.5 rounded-[3px] border border-border/60 bg-surface-dim/30 p-3">
              ...
              <label className="flex items-center gap-3 rounded-[3px] border border-border/60 bg-surface-dim/30 px-4 py-3 cursor-pointer hover:bg-primary/5 transition-colors sm:col-span-1 sm:mt-2">
                <input
                  type="checkbox"
              ...
              <div className="col-span-2 rounded-[3px] border border-primary/20 bg-primary/5 p-4 space-y-3">
          ...
          <section className="rounded-[3px] border border-[#ebecf0] bg-white p-5 sm:p-6 space-y-3 shadow-sm">
          ...
          <section className="rounded-[3px] border border-[#ebecf0] bg-[#f4f5f7]/30 p-5 sm:p-6 space-y-4 shadow-sm">
            <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-2">
            ...
            <div className="inline-flex p-1 rounded-[3px] bg-white border border-[#dfe1e6]/80 shadow-sm w-full sm:w-auto">
              <button
                type="button"
            ...
            {editing && analytics && (
              <div className="text-xs text-slate-600 flex flex-wrap gap-x-6 gap-y-2 rounded-[3px] border border-[#dfe1e6]/60 bg-white/80 px-4 py-3">
                <span>
          ...
          <section className="rounded-[3px] border border-emerald-100/80 bg-gradient-to-br from-emerald-50/50 to-white p-5 sm:p-6 space-y-3 shadow-sm">
          ...
          {form.editorMode === "canvas" && form.canvasGraph && (
            <section className="rounded-[3px] border border-violet-100 bg-violet-50/20 p-5 sm:p-6 shadow-sm space-y-3">
              <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-2">
              ...
              <div className="rounded-[3px] border border-violet-200/60 bg-white/90 p-3 space-y-2">
                <label className="block text-xs font-semibold text-slate-700">Start from a template</label>
                <select
                  className="w-full max-w-xl rounded-[3px] border border-[#dfe1e6] px-3 py-2 text-sm bg-white"
                  defaultValue=""
                  onChange={(e) => {
          ...
          {form.editorMode === "branches" && (
            <section className="rounded-[3px] border border-[#ebecf0] bg-white p-5 sm:p-6 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-[3px] bg-primary text-primary-foreground text-sm font-semibold shadow-sm hover:bg-primary-dark shrink-0"
                  onClick={() =>
                    setForm((f) => ({
                      ...f,
                      branches: [
              ...
              <div className="space-y-5">
                {form.branches.map((br, bi) => (
                  <div key={bi} className="rounded-[3px] border border-[#dfe1e6]/80 p-5 space-y-4 bg-gradient-to-b from-slate-50/40 to-white shadow-sm">
                    <div className="flex flex-wrap gap-2 items-center">
                      <input
                        className="font-bold border border-[#dfe1e6] rounded-[3px] px-3 py-1.5 flex-1 min-w-[160px] bg-white"
                        value={br.label}
                        onChange={(e) => {
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {br.steps.map((step, si) => (
                          <div key={si} className="border border-white shadow-sm rounded-[3px] p-3 bg-white">
                            {step.kind === "delay" ? (
                              <div className="flex flex-wrap gap-2 items-end">
                                ...
                                <input
                                  type="number"
                                  min={0}
                                  className="w-16 border rounded-lg px-2 py-1 text-sm border-[#dfe1e6] bg-white"
                                  value={step.days}
                                  onChange={(e) => {
                                    const next = [...form.branches];
                                    const ns = [...br.steps];
                              ...
                              <div>
                                <div className="flex gap-2 mb-2">
                                  <select
                                    className="border border-[#dfe1e6] rounded-[3px] px-2 py-1 text-sm font-medium flex-1 bg-white"
...
function ActionFields({
  ...
  if (t === "assign_owner") {
    return (
      <input
        className="border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm w-full"
  ...
  if (t === "create_task") {
    return (
      <div className="grid gap-2">
        <input
          className="border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm"
          placeholder="Title"
        ...
        <input
          type="number"
          min={0}
          className="border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm w-40"
  ...
  if (t === "create_note") {
    return (
      <textarea
        className="border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm w-full min-h-[80px]"
  ...
  if (t === "notify_teams") {
    return (
      <div className="grid gap-2">
        <p className="text-xs text-amber-900/90 rounded-[3px] border border-amber-200/80 bg-amber-50/80 px-3 py-2 leading-snug">
        ...
        />
        <textarea
          className="border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm min-h-[72px]"
