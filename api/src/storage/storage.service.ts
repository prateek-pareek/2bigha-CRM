...
export class StorageService {
  ...
  /**
   * Same as `uploadDocument`, but for server-generated bytes (e.g. a
   * puppeteer-rendered PDF) rather than a multipart `Express.Multer.File` —
   * mirrors how `uploadBuffer` relates to `uploadFile` for images. Always
   * stored under `uploads/files/`, same as `uploadDocument`, since that's the
   * only subfolder `MediaUploadsController.serveFile` (`GET /uploads/files/:filename`)
  ...
  async uploadDocumentBuffer(
    ...
    assertAllowedFileMime(mimeType);
    if (buffer.length > this.maxFileBytes) {
      throw new BadRequestException(
        `File is too large (${formatBytes(buffer.length)}). Max ${formatBytes(this.maxFileBytes)}.`,
      );
    }

    const mime = (mimeType || 'application/octet-stream').split(';')[0].trim();
    const ext = extname(originalName || '') || `.${fileExtFromMime(mime)}`;
    const storedName = this.buildFilename(ext.replace(/^\./, ''), originalName);
