...
import {
  ...
  parseInvalidEmailLog,
} from '../shared/crm-undeliverable.util';
...
export class InboxAccountsService {
  ...
  /** List CRM records with flagged undeliverable / invalid email addresses. */
  async listUndeliverableContacts(): Promise<{
    total: number;
    items: Array<{
      email: string;
      reason: string | null;
    ...
    const filter = { invalidEmails: { $exists: true, $ne: [] } };
    const [leads, contacts, clients] = await Promise.all([
      this.leadModel
        .find(filter)
        .select(
          'firstName lastName email additionalEmails invalidEmails customFields',
        )
        .lean()
        .exec(),
      this.contactModel
      ...
      this.clientModel
        .find(filter)
        .select('name email additionalEmails invalidEmails customFields')
        .lean()
        .exec(),
    ]);
    ...
    const byEmail = new Map<string, { records: Rec[] }>();

    const add = (rec: Rec) => {
      for (const inv of rec.invalidEmails) {
        const em = String(inv || '').trim().toLowerCase();
        if (!em.includes('@')) continue;
        const bucket = byEmail.get(em) || { records: [] };
        if (!bucket.records.some((r) => r.entityId === rec.entityId && r.module === rec.module)) {
    ...
    for (const l of leads) {
      const invalid = ((l as { invalidEmails?: string[] }).invalidEmails || []).map(
        (e) => String(e).trim().toLowerCase(),
      );
      const primary = String(l.email || '').trim() || null;
      add({
        module: 'leads',
        entityId: String(l._id),
        label: `${l.firstName || ''} ${l.lastName || ''}`.trim() || 'Lead',
        primaryEmail: primary,
        additionalEmails: (l.additionalEmails || []).map(String),
        invalidEmails: invalid,
        primaryIsInvalid: !!primary && invalid.includes(primary.toLowerCase()),
        customFields: (l as { customFields?: Record<string, unknown> }).customFields,
      });
    }
    for (const c of contacts) {
      const invalid = ((c as { invalidEmails?: string[] }).invalidEmails || []).map(
        (e) => String(e).trim().toLowerCase(),
      );
      const primary = String(c.email || '').trim() || null;
      add({
        module: 'contacts',
        entityId: String(c._id),
        label: `${c.firstName || ''} ${c.lastName || ''}`.trim() || 'Contact',
        primaryEmail: primary,
        additionalEmails: (c.additionalEmails || []).map(String),
        invalidEmails: invalid,
        primaryIsInvalid: !!primary && invalid.includes(primary.toLowerCase()),
    ...
    for (const cl of clients) {
      const invalid = ((cl as { invalidEmails?: string[] }).invalidEmails || []).map(
        (e) => String(e).trim().toLowerCase(),
      );
      const primary = String(cl.email || '').trim() || null;
      add({
        module: 'clients',
        entityId: String(cl._id),
        label: String(cl.name || 'Client'),
        primaryEmail: primary,
        additionalEmails: (cl.additionalEmails || []).map(String),
        invalidEmails: invalid,
        primaryIsInvalid: !!primary && invalid.includes(primary.toLowerCase()),
        customFields: (cl as { customFields?: Record<string, unknown> }).customFields,
      });
    }

    const items = [...byEmail.entries()]
      .map(([email, { records }]) => {
        const logSource = records.find((r) =>
          (r.invalidEmails || []).includes(email),
        );
        const { reason, flaggedAt } = parseInvalidEmailLog(
          logSource?.customFields,
          email,
        );
        return {
          email,
          reason,
          flaggedAt,
          records: records.map(({ customFields: _cf, ...rest }) => rest),
        };
      })
      .sort((a, b) => {
        const ta = a.flaggedAt ? Date.parse(a.flaggedAt) : 0;
        const tb = b.flaggedAt ? Date.parse(b.flaggedAt) : 0;
        return tb - ta;
      });
  ...
  /** Manually flag an address as undeliverable (admin). */
  async flagEmailUndeliverable(
    email: string,
    reason?: string,
  ): Promise<{ matchedRecords: number }> {
    const trimmed = String(email || '').trim();
    if (!trimmed.includes('@')) {
      throw new BadRequestException('Valid email required');
    }
    const matched = await this.markEmailInvalidAcrossRecords(
      trimmed,
      reason?.trim() || 'Manually marked undeliverable',
    );
    return { matchedRecords: matched };
  }

  /**
   * Clear bad email from a record or delete the CRM record entirely.
   */
  async resolveUndeliverableContact(body: {
    module: 'leads' | 'contacts' | 'clients';
    entityId: string;
    email: string;
    ...
    if (!['leads', 'contacts', 'clients'].includes(module)) {
    ...
    if (!['clear_email', 'remove_contact', 'allow_retry'].includes(action)) {
    ...
    if (module === 'leads') {
      const doc = await this.leadModel.findById(oid).exec();
      if (!doc) throw new NotFoundException('Record not found');
      if (action === 'remove_contact') {
        await this.leadModel
          .findByIdAndUpdate(oid, softDeleteUpdate(), { new: true })
          .exec();
        return { success: true, message: 'Lead moved to Trash' };
      }
      if (action === 'allow_retry') {
        await this.leadModel.updateOne({ _id: oid }, { $pull: { invalidEmails: regex } }).exec();
        return {
          success: true,
          message: 'Address removed from undeliverable list — you can try sending again',
        };
      }
      const primary = String(doc.email || '').trim();
      const additional = (doc.additionalEmails || []).map((e) => String(e).trim());
      const remainingAdditional = additional.filter((e) => e.toLowerCase() !== email);
      const nextPrimary =
        primary.toLowerCase() === email
          ? remainingAdditional.find((e) => e.includes('@')) || ''
          : primary;
      await this.leadModel
        .updateOne(
          { _id: oid },
          { $set: { email: nextPrimary }, $pull: { additionalEmails: regex, invalidEmails: regex } },
        )
        .exec();
      return {
        success: true,
        message: nextPrimary
          ? 'Invalid email cleared; primary updated'
          : 'Invalid email cleared; record has no primary email now',
      };
    }

    if (module === 'contacts') {
      const doc = await this.contactModel.findById(oid).exec();
      if (!doc) throw new NotFoundException('Record not found');
      if (action === 'remove_contact') {
        await this.contactModel
          .findByIdAndUpdate(oid, softDeleteUpdate(), { new: true })
          .exec();
        return { success: true, message: 'Contact moved to Trash' };
      }
      if (action === 'allow_retry') {
        await this.contactModel.updateOne({ _id: oid }, { $pull: { invalidEmails: regex } }).exec();
        return {
          success: true,
          message: 'Address removed from undeliverable list — you can try sending again',
        };
    ...
    const doc = await this.clientModel.findById(oid).exec();
    if (!doc) throw new NotFoundException('Record not found');
    if (action === 'remove_contact') {
    ...
    if (action === 'allow_retry') {
      await this.clientModel.updateOne({ _id: oid }, { $pull: { invalidEmails: regex } }).exec();
      return {
        success: true,
        message: 'Address removed from undeliverable list — you can try sending again',
      };
  ...
  /**
   * Bulk-remove CRM records flagged as undeliverable (leads / contacts / clients).
   */
  async bulkDeleteUndeliverableContacts(
    items: Array<{
      module: 'leads' | 'contacts' | 'clients';
      entityId: string;
      email?: string;
    ...
    // Deduplicate by module+entityId (same record can appear under multiple bad emails).
    const seen = new Set<string>();
    const unique: Array<{ module: 'leads' | 'contacts' | 'clients'; entityId: string; email: string }> =
      [];
    for (const raw of items) {
      const module = raw?.module;
      const entityId = String(raw?.entityId || '').trim();
      if (!['leads', 'contacts', 'clients'].includes(module)) continue;
      if (!Types.ObjectId.isValid(entityId)) continue;
      ...
      unique.push({
        module,
        entityId,
        email: String(raw?.email || 'bulk@delete.local').trim().toLowerCase() || 'bulk@delete.local',
      });
    }

    if (unique.length === 0) {
      throw new BadRequestException('No valid records to delete');
    }
    ...
    for (const item of unique) {
      try {
        await this.resolveUndeliverableContact({
          module: item.module,
          entityId: item.entityId,
          email: item.email.includes('@') ? item.email : 'bulk@delete.local',
    ...
    return {
      ...
      message:
        failed === 0
          ? `Removed ${deleted} record${deleted === 1 ? '' : 's'} from CRM`
          : `Removed ${deleted} record${deleted === 1 ? '' : 's'}; ${failed} failed`,
    };
  }
