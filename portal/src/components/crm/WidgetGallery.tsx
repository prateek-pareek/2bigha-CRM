...
export default function WidgetGallery({ isOpen, onClose, onAdd, context = 'dashboard' }: WidgetGalleryProps) {
 ...
 <div className={cn(crmModalChrome.slidePanel, 'max-w-md crm-jira-modal')}>
 ...
 type="button"
 key={widget.id}
 className="group w-full rounded-[3px] border border-[#dfe1e6] bg-white p-4 text-left transition-colors hover:border-[#0c66e4] hover:bg-[#fafbfc]"
 onClick={() => onAdd(widget.type, widget.component, widget.title)}
 >
 <div className="flex items-start gap-3">
 <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[3px] border border-[#dfe1e6] bg-[#f4f5f7] text-[#0c66e4]">
 {widget.icon}
 </div>
 <div className="min-w-0 flex-1">
 <h3 className="text-sm font-medium text-[#172b4d]">{widget.title}</h3>
 <p className="mt-0.5 text-xs leading-relaxed text-[#5e6c84]">{widget.description}</p>
 </div>
 <LayoutGrid size={14} className="mt-1 shrink-0 text-[#97a0af] group-hover:text-[#0c66e4]" />
 </div>
 </button>
 ))}
 </div>
