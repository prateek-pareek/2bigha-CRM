...
export default function TemplateModal({ isOpen, onClose, template, onSuccess }: TemplateModalProps) {
  ...
  const modalContent = (
    <div className={crmModalChrome.overlay}>
      ...
      <div className={`${crmModalChrome.slidePanel} max-w-4xl crm-jira-modal`}>
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className={`${crmModalChrome.slideBody} space-y-6`}>
             <div className="space-y-2">
              ...
              <input
                ...
                className="block w-full rounded-[3px] border-border bg-surface-dim/30 text-sm font-bold text-text-main focus:bg-card focus:border-primary/30 focus:ring-4 focus:ring-primary/5 h-12 border px-5 transition-all outline-none"
                required
              />
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {steps.map((step, index) => (
                  <div key={step.id} className="group relative bg-card border border-[#ebecf0] rounded-[24px] p-6 shadow-sm hover:shadow-md transition-all border-l-4 border-l-primary/20 hover:border-l-primary">
                    <div className="flex items-start gap-6">
                      ...
                      <div className="flex-1 space-y-4">
                        <div className="flex flex-wrap items-center gap-4">
                          <div className="flex items-center gap-2 bg-surface-dim px-3 py-1.5 rounded-[3px] border border-[#ebecf0]">
                          ...
                          <button 
                            ...
                            className="ml-auto p-2 opacity-0 group-hover:opacity-100 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-[3px] transition-all"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
                        ...
                        {step.type === 'Email' && (
                          <input
                            ...
                            className="block w-full rounded-[3px] border-border bg-surface-dim/30 text-xs font-bold text-text-main focus:bg-card focus:border-primary/30 h-10 border px-4 transition-all outline-none"
                          />
                        )}
                        ...
                        <textarea
                          ...
                          className="block w-full rounded-[3px] border-border bg-surface-dim/30 text-xs font-medium text-text-main focus:bg-card focus:border-primary/30 min-h-[80px] border px-4 py-3 transition-all outline-none leading-relaxed resize-none"
                        />
                      </div>
                    </div>
                  </div>
                ...
                {steps.length === 0 && (
                  <div className="py-12 border-2 border-dashed border-[#ebecf0] rounded-[32px] flex flex-col items-center justify-center opacity-40">
            ...
            <div className="bg-primary/5 rounded-[24px] p-5 flex items-start gap-4 border border-primary/10">
          ...
          <div className={crmModalChrome.slideFooter}>
            <button
              type="submit"
              disabled={loading}
              className="inline-flex h-8 w-full items-center justify-center gap-2 rounded-[3px] bg-[#0c66e4] px-4 text-sm font-medium text-white hover:bg-[#0055cc] disabled:opacity-50"
            >
