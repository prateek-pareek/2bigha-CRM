...
export default function CRMLeadFormFields({
  ...
  const renderField = (key: string) => {
    switch (key) {
      ...
      case 'firstName':
        return (
          <div key={key}>
            <label className={LBL}>First name <span className="text-[#f2545b]">*</span></label>
      ...
      <div key={key}>
        <div className="flex items-center justify-between mb-1">
          <label className={LBL + ' mb-0'}>
            {field.name}{required && <span className="text-[#f2545b] ml-0.5">*</span>}
          </label>
  ...
  // Stack variant — group by section
    ...
    <div className="space-y-3">
          ...
          <div className="space-y-2">
            {group.keys.map((key) => {
              if (namePair && key === nameLeader) {
                return (
                  <div key="name-pair" className="grid grid-cols-2 gap-3">
                    {renderField('firstName')}
                    {renderField('lastName')}
                  </div>
                );
              }
              return key.startsWith('cf:') ? renderCustomField(key) : renderField(key);
            })}
          </div>
        </div>
      ))}
      <div className="pt-3 border-t border-[var(--border-color)]/80 space-y-2">
        ...
        <p className="text-xs text-[var(--text-muted)] leading-snug">
          For outreach via Upwork, Naukri, Indeed, etc. when you do not have an email or phone yet — paste the public listing or project URL (must start with https://).
        </p>
