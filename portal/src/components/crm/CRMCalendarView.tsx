...
export default function CRMCalendarView({ items, onItemClick, dateField = 'createdAt', titleField }: CRMCalendarViewProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-[32px] border border-border overflow-hidden shadow-soft animate-in fade-in duration-500">
      <div className="p-6 border-b border-border bg-surface-dim/30 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-[3px] bg-primary/10 flex items-center justify-center text-primary">
            <CalendarIcon size={20} />
          </div>
          <div>
            <h3 className="text-lg font-bold text-text-main tracking-tight">{monthName} {year}</h3>
            <p className="text-xs font-medium text-text-muted">Outlook Calendar View</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={prevMonth} className="p-2.5 hover:bg-card border border-transparent hover:border-border rounded-[3px] text-text-muted hover:text-text-main transition-all">
            <ChevronLeft size={20} />
          </button>
          <button onClick={() => setCurrentDate(new Date())} className="px-4 py-2 bg-card border border-border rounded-[3px] text-xs font-bold text-text-main hover:bg-surface-dim transition-all">
            Today
          </button>
          <button onClick={nextMonth} className="p-2.5 hover:bg-card border border-transparent hover:border-border rounded-[3px] text-text-muted hover:text-text-main transition-all">
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        <div className="grid grid-cols-7 border-b border-border bg-surface-dim/10 shrink-0">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div key={day} className="py-4 text-center text-xs font-black text-text-muted border-r border-border last:border-0">
              {day}
            </div>
          ))}
        </div>
              ...
              <div key={idx} className={cn(
                ...
                {day && (
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {dayItems.length > 0 && (
                      <span className="text-[9px] font-black text-primary px-1.5 py-0.5 bg-primary/10 rounded-md uppercase">
