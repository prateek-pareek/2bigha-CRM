...
export default function WorkflowSendEmailActionFields({
  ...
  const sel = isForm
    ? `border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm shadow-sm ${focusRing}`
    : "nodrag nopan w-full rounded-lg border border-[#ebecf0] py-1 px-1 text-[9px] font-semibold";
  ...
  const numCls = isForm
    ? `border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm w-full max-w-[200px] shadow-sm ${focusRing}`
    : "nodrag nopan w-full rounded-lg border border-[#ebecf0] px-1 py-0.5 text-[9px]";
    ...
    <div className={isForm ? "grid gap-3" : "mt-1 space-y-1"}>
      ...
      {sendMode === "template" ? (
        ...
        <label className="block">
          ...
          <textarea
            className={isForm ? "mt-0.5 min-h-[72px] w-full rounded-[3px] border border-[#dfe1e6] bg-white px-2 py-1.5 text-sm" : "nodrag nopan min-h-[48px] w-full rounded-lg border border-[#ebecf0] px-1 py-0.5 text-[9px]"}
      ...
      <div className={isForm ? "grid gap-1.5 rounded-[3px] border border-[#dfe1e6]/70 bg-[#f4f5f7]/30 p-3" : "flex flex-col gap-0.5"}>
        <span className={lab}>Sender</span>
        <select
          className={isForm ? "border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm w-full bg-white" : numCls}
      ...
      {!useSplit && (
        <label className={isForm ? "block rounded-[3px] border border-[#dfe1e6]/70 bg-[#f4f5f7]/30 p-3" : "block"}>
          <span className={lab}>Send from mailbox</span>
          <select
            className={isForm ? "mt-0.5 w-full border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm bg-white" : numCls}
      ...
      {useSplit && (
        <div className={isForm ? "grid gap-2 rounded-[3px] border border-violet-100/80 bg-gradient-to-br from-violet-50/40 to-slate-50/30 p-3" : "rounded border border-[#ebecf0] bg-[#f4f5f7]/80 px-1 py-1"}>
          <span className={lab}>Split mode</span>
          <select
            className={isForm ? "border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm" : numCls}
      ...
      <label className={isForm ? "block rounded-[3px] border border-[#dfe1e6]/70 bg-[#f4f5f7]/30 px-3 py-2.5" : "block"}>
        <span className={lab}>Send jitter (seconds)</span>
        <input
          type="number"
          min={0}
          max={3600}
          className={isForm ? "mt-0.5 border border-[#dfe1e6] rounded-[3px] px-2 py-1.5 text-sm w-full max-w-[220px] bg-white" : numCls}
          placeholder="0"
      ...
      <div className={isForm ? "rounded-[3px] border border-[#dfe1e6]/70 bg-[#f4f5f7]/40 px-3 py-2.5" : "rounded border border-[#ebecf0] px-1 py-1"}>
      ...
      <details className={isForm ? "rounded-[3px] border border-[#dfe1e6]/70 bg-[#f4f5f7]/30 px-3 py-2 shadow-sm" : "rounded border border-[#ebecf0] px-1 py-0.5"}>
      ...
      {isForm && (
        <>
          ...
          <label className="flex items-center gap-2 text-xs text-slate-600 rounded-md bg-[#f4f5f7] px-2 py-1">
            <input
              type="checkbox"
