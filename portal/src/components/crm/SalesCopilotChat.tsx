    ...
    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50/80 p-3">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          onClick={() => void reject()}
          disabled={!!loading}
          className="inline-flex items-center gap-1 rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-[#f4f5f7] disabled:opacity-50"
        >
...
export function SalesCopilotChat({
  ...
  return (
    <div className={cn('flex min-h-0 flex-col', className)}>
        ...
        <div className={cn('shrink-0', compact ? 'mb-3' : 'mb-4')}>
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-[3px] bg-violet-500/10 text-violet-600">
                <Bot size={20} />
              </div>
              <div>
      ...
      {configured === false ? (
        <div className="mb-3 flex gap-3 rounded-[3px] border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm text-amber-900 shrink-0">
      ...
      <div
        ref={scrollRef}
        className={cn(
          'flex-1 min-h-0 overflow-y-auto rounded-[3px] border border-[var(--border-color)] bg-white shadow-sm',
          compact ? 'min-h-[280px]' : '',
        )}
      >
        {messages.length === 0 ? (
      ...
      >
        <input
          ...
          className="flex-1 rounded-[3px] border border-[var(--border-color)] bg-white px-4 py-2.5 text-sm text-[var(--text-main)] outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
        />
        <button
          type="submit"
          disabled={loading || configured === false || !input.trim()}
          className="inline-flex items-center justify-center rounded-[3px] bg-violet-600 px-4 py-2.5 text-white hover:bg-violet-700 disabled:opacity-50"
        >
