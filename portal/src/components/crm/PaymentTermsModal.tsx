...
const fieldClass =
  'w-full h-8 rounded-[3px] border border-[#dfe1e6] bg-white px-3 text-sm font-normal text-[#172b4d] outline-none focus-visible:border-[#0c66e4] focus-visible:ring-1 focus-visible:ring-[#0c66e4]/30';
const labelClass = 'text-xs font-semibold text-[#5e6c84]';
...
export default function PaymentTermsModal({ isOpen, onClose, dealId }: PaymentTermsModalProps) {
 ...
 <div className={cn(crmModalChrome.centerShell, 'max-w-3xl max-h-[min(90vh,56rem)] crm-jira-modal flex flex-col')}>
 ...
 {terms.map((term, idx) => (
 <div key={term._id || idx} className="grid grid-cols-12 items-end gap-3 rounded-[3px] border border-[#dfe1e6] bg-[#fafbfc] p-4">
 ...
 <DollarSign className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#5e6c84]" size={14} />
 <input
 type="number"
 ...
 buttonClassName="h-8 w-full justify-start rounded-[3px] border-[#dfe1e6] bg-white px-3 text-sm font-normal"
 />
 </div>
 ...
 disabled={saving}
 className="inline-flex h-8 w-8 items-center justify-center rounded-[3px] bg-[#0c66e4] text-white hover:bg-[#0055cc]"
 >
 ...
 className="inline-flex h-8 w-8 items-center justify-center rounded-[3px] border border-[#dfe1e6] bg-white text-[#5e6c84] hover:bg-[#ffebe6] hover:text-[#de350b]"
 >
 ...
 className="flex w-full items-center justify-center gap-2 rounded-[3px] border border-dashed border-[#dfe1e6] py-5 text-sm font-medium text-[#5e6c84] transition-colors hover:border-[#0c66e4] hover:bg-[#deebff]/40 hover:text-[#0c66e4]"
 >
