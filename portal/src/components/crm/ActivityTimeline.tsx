...
export default function ActivityTimeline({
 ...
 {[1, 2, 3].map(i => (
 <div key={i} className="bg-surface-dim h-32 rounded-[32px] border border-[#ebecf0]" />
 ))}
 </div>
 );
 }
 ...
 if (isSentEmailActivity(activity)) return 'bg-blue-50';
 switch (activity.type) {
 ...
 <div className="w-12 h-12 bg-card border-2 border-[#ebecf0] rounded-[3px] flex items-center justify-center text-text-muted shadow-sm">
 <Calendar size={18} />
 </div>
 ...
 <div key={activity._id} className="group relative bg-card p-7 rounded-[32px] border border-[#ebecf0] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
 ...
 <div className={`w-10 h-10 ${getBg(activity)} rounded-[3px] flex items-center justify-center`}>
 {getIcon(activity)}
 </div>
 <div>
 ...
 {onEdit && (
 <button type="button" onClick={() => onEdit(activity)} className="p-2 text-text-muted hover:text-primary hover:bg-primary/5 rounded-[3px] transition-all"><Edit2 size={16} /></button>
 )}
 {allowDelete && onDelete && (
 <button type="button" onClick={() => onDelete(activity._id)} className="p-2 text-text-muted hover:text-rose-500 hover:bg-rose-50 rounded-[3px] transition-all"><Trash2 size={16} /></button>
 )}
 </div>
 </div>
 ...
 {activity.type === 'Email' && activity.metadata ? (
   <div className="space-y-3">
     <div className="font-bold text-slate-900 border-b border-[#ebecf0] pb-2">
       Subject: {activity.metadata.subject || activity.title}
     </div>
     {activity.metadata.direction === 'inbound' ? (
       <div className="space-y-1 rounded-[3px] border border-[#dfe1e6] bg-[#f4f5f7]/90 px-3 py-2 text-xs font-semibold text-slate-700">
         <p>
     ...
     {activity.metadata.bodyHtml ? (
       <iframe
         title="Email body"
         className="w-full min-h-[280px] rounded-[3px] border border-[#dfe1e6] bg-white"
         sandbox=""
         srcDoc={buildCrmEmailPreviewSrcDoc(activity.metadata.bodyHtml)}
       />
     ) : activity.metadata.bodyPlain ? (
       <pre className="max-h-[min(70vh,520px)] overflow-auto whitespace-pre-wrap rounded-[3px] border border-[#dfe1e6] bg-white p-4 text-sm leading-relaxed text-slate-800">
         {activity.metadata.bodyPlain}
       </pre>
     ) : (
       <div className="whitespace-pre-wrap rounded-[3px] border border-[#dfe1e6] bg-[#f4f5f7] p-4 text-sm leading-relaxed text-slate-700">
         {activity.metadata?.direction === 'inbound'
   ...
   <div className="space-y-2">
     ...
     {(activity.metadata?.bodyHtml || activity.metadata?.bodyPlain) && (
       <details className="rounded-[3px] border border-[#dfe1e6] bg-[#f4f5f7]/80">
         <summary className="cursor-pointer list-none px-3 py-2 text-xs font-bold uppercase tracking-wider text-primary">
           View full email
         </summary>
         <div className="px-3 pb-3 border-t border-[#ebecf0]">
           {activity.metadata?.bodyHtml ? (
             <iframe
               title="Sent email preview"
               className="mt-2 w-full min-h-[240px] rounded-lg border border-[#dfe1e6] bg-white"
               sandbox=""
               srcDoc={buildCrmEmailPreviewSrcDoc(activity.metadata.bodyHtml)}
             />
           ) : (
             <pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap rounded-lg border border-[#dfe1e6] bg-white p-3 text-xs leading-relaxed">
               {activity.metadata?.bodyPlain}
             </pre>
           )}
         </div>
 ...
 <div className="px-3 py-1 bg-surface-dim text-xs font-black text-text-muted rounded-full border border-[#ebecf0]">
