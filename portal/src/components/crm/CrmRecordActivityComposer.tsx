...
import { formatCrmUserLabel, type CrmPortalUserOption } from "@/components/crm/ActivityLogger";
...
export default function CrmRecordActivityComposer({
  ...
  return (
    <div className="mb-6 theme-crm-hubspot">
      {/* Section header - sits above tabs */}
      <h3 className="text-xs font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mb-3 px-0.5">
        Activity Timeline
      </h3>

      {/* Tab Navigation */}
      <div className="flex bg-surface-dim p-1 rounded-[3px] border border-border flex-wrap gap-1 mb-5">
        {types.map((type) => (
          <button
            key={type}
            type="button"
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${
              activityType === type
                ? "bg-[var(--card-bg)] text-[var(--text-main)] shadow-sm border border-border italic"
      ...
      {activityType === "Meeting" ? (
        <div className="rounded-[3px] border border-border bg-card p-6 shadow-sm space-y-4">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-[3px] border border-primary/15 bg-primary/10 flex items-center justify-center text-primary shrink-0">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-[3px] bg-primary text-primary-foreground text-sm font-bold hover:bg-primary/90 transition-all shadow-md shadow-primary/20 active:scale-[0.99]"
          >
            <Video size={18} />
        ...
        <div className="space-y-4 rounded-[3px] border border-border bg-card p-5 shadow-sm">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full rounded-[3px] border border-border/60 bg-surface-dim/30 px-4 py-3 text-sm font-semibold text-text-main placeholder:text-text-muted/60 focus:ring-2 focus:ring-primary/20 focus:border-primary/30 outline-none transition-all"
            />
          </div>
          <div className="space-y-1.5">
            <label className="block text-xs font-black text-text-muted px-1">
            ...
            <textarea
              className="w-full rounded-[3px] border border-border/60 bg-surface-dim/30 p-4 text-sm font-medium text-text-main placeholder:text-text-muted/60 focus:ring-2 focus:ring-primary/20 focus:border-primary/30 outline-none transition-all resize-none min-h-[100px]"
          ...
          <div className="space-y-1.5 max-w-xs">
            ...
            <DatePickerField
              ...
              buttonClassName="h-11 rounded-[3px] border-border/60 bg-surface-dim/30 hover:bg-surface-dim/50"
            />
          </div>
          {crmUsers.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <select
                  className="w-full rounded-[3px] border border-border/60 bg-surface-dim/30 px-3 py-2.5 text-xs font-semibold text-text-main outline-none focus:ring-2 focus:ring-primary/20"
          ...
          <div className="flex justify-end pt-1 border-t border-border/40">
            <button
              ...
              className="inline-flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-[3px] hover:bg-primary-dark transition-all shadow-md active:scale-95 text-xs font-semibold disabled:opacity-45 disabled:pointer-events-none"
            >
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
          {isFetchingMeta ? (
            ...
            <div className="text-center py-12 bg-surface-dim/30 rounded-[3px] border border-dashed border-border">
        ...
        <div className="space-y-4 rounded-[3px] border border-border bg-card p-5 shadow-sm">
          ...
          <textarea
            className="w-full border border-border/60 rounded-[3px] p-4 text-sm font-medium bg-surface-dim/30 focus:ring-2 focus:ring-primary/20 outline-none transition-all resize-none min-h-[88px]"
          ...
          <div className="flex justify-end pt-1 border-t border-border/40">
            <button
              type="button"
              onClick={handlePostActivity}
              className="inline-flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-[3px] hover:bg-primary-dark transition-all shadow-md active:scale-95 text-xs font-semibold"
            >
        ...
        <div className="relative space-y-2">
          <textarea
            className={`w-full border rounded-[3px] p-4 text-sm font-medium focus:ring-2 outline-none transition-all resize-none h-24 ${
              activityType === "Call"
                ? "bg-success-light/30 border-success/20 focus:ring-success/20"
                : activityType === "Note"
                  ? "bg-amber-50/40 border-amber-200/50 dark:bg-amber-950/15 dark:border-amber-800/30 focus:ring-amber-500/20"
                  : "bg-surface-dim border-border focus:ring-primary/20"
            }`}
            placeholder={
              activityType === "Call"
                ? "Log call notes..."
          ...
          <button
            type="button"
            onClick={handlePostActivity}
            className="absolute bottom-3 right-3 bg-primary text-white p-2.5 rounded-[3px] hover:bg-primary-dark transition-all shadow-md active:scale-95"
