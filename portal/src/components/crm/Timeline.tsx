...
export default function Timeline({
  ...
  useEffect(() => {
    setLocalItems(normalizeActivities(activities));
  }, [activities]);
    ...
    <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border/50 before:to-transparent">
      {items.map((activity) => (
        <div key={activity._id} className="relative flex items-start group">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[3px] bg-card border border-border shadow-sm z-10 group-hover:scale-110 group-hover:shadow-md transition-all duration-300">
            {getIcon(activity)}
          </div>
          <div className="ml-5 flex-1 animate-in slide-in-from-left-2 duration-300">
            <div className="bg-card p-5 rounded-[24px] border border-border shadow-sm group-hover:shadow-lg group-hover:border-primary/10 transition-all duration-300">
              ...
              {activity.type === 'Task' ? (
                ...
                <div className="space-y-3">
                  ...
                  activity.metadata?.fromEmail ? (
                    <p className="mb-1 rounded-[3px] border border-border/60 bg-surface-dim/40 px-3 py-2 text-xs font-medium leading-snug text-text-muted">
                  ...
                  {isInboundEmailTimeline(activity) ? (
                    <div className="rounded-[3px] border border-border/70 bg-surface-dim/40 px-3 py-2 text-xs font-semibold text-text-main">
                    ...
                    <div className="rounded-[3px] border border-border/70 bg-surface-dim/40 px-3 py-2 text-xs font-semibold text-text-main space-y-1">
                      {activity.metadata.fromEmail ? (
                        <div>
                          <span className="text-text-muted">From: </span>
                          {activity.metadata.fromEmail}
                  ...
                  {activity.metadata.bodyHtml ? (
                    <iframe
                      title="Email message"
                      className="w-full min-h-[260px] rounded-[3px] border border-border bg-white"
                      sandbox=""
                      srcDoc={buildCrmEmailPreviewSrcDoc(
                        activity.metadata.bodyHtml,
                      )}
                    />
                  ) : activity.metadata.bodyPlain ? (
                    <pre className="max-h-[min(60vh,400px)] overflow-auto whitespace-pre-wrap rounded-[3px] border border-border bg-card p-3 text-sm leading-relaxed text-text-main">
                      {activity.metadata.bodyPlain}
                    </pre>
                  ) : (
                    <div className="whitespace-pre-wrap rounded-[3px] border border-border bg-surface-dim/30 p-3 text-sm leading-relaxed text-text-main">
                      {isInboundEmailTimeline(activity)
                  ...
                  {Array.isArray(activity.metadata.attachments) &&
                    activity.metadata.attachments.length > 0 && (
                      <div className="flex flex-wrap gap-2 pt-2 border-t border-border/40 mt-3">
                        {activity.metadata.attachments.map((att: any) => {
                          const emailIdForDownload = activity.metadata.inboxEmailId || activity.metadata.emailId;
                          const downloadUrl = emailIdForDownload
                            ? `${CRM_API_URL}/crm/inbox-accounts/emails/${encodeURIComponent(emailIdForDownload)}/attachments/${encodeURIComponent(att.id)}`
                            : null;
                          
                          return (
                            <div
                              key={att.id}
                              className="group/att flex items-center gap-2 rounded-[3px] border border-border bg-surface-dim/20 px-3 py-2 text-xs font-bold text-text-main hover:bg-surface-dim/40 hover:border-primary/20 transition-all"
                            >
                              ...
                              {downloadUrl && (
                                <button
                                  type="button"
                                  onClick={() => handleDownloadAttachment(emailIdForDownload, att)}
                                  className="ml-1 rounded-md bg-primary/10 px-2 py-0.5 text-xs font-black uppercase text-primary hover:bg-primary hover:text-white transition-all"
                                >
                                  Download
                                </button>
                              )}
                ...
                <div className="space-y-2">
                  ...
                  {activity.metadata?.workflowEmailSent ? (
                    <p className="rounded-[3px] border border-indigo-200 bg-indigo-50/80 px-3 py-2 text-xs font-semibold text-indigo-900">
              ...
              {activity.type === 'Meeting' && activity.metadata && (
                <div className="mt-4 p-4 bg-surface-dim/40 rounded-[3px] border border-border/50 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {activity.metadata.meetingLink && (
                    <div className="col-span-1 sm:col-span-2 mt-1">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-[3px] text-xs font-black uppercase tracking-wider hover:bg-primary/90 transition-all shadow-sm shadow-primary/20"
                      >
                        <Video size={12} />
                        Join Meeting
                      </a>
      ...
      {items.length === 0 && (
        <div className="text-center py-20 bg-surface-dim/20 rounded-[32px] border border-dashed border-border/50">
          <div className="w-16 h-16 rounded-[3px] bg-surface-dim flex items-center justify-center mx-auto mb-4">
