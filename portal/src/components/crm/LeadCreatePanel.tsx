...
import CRMLeadFormFields from "@/components/crm/CRMLeadFormFields";
import CRMContactFormFields from "@/components/crm/CRMContactFormFields";
...
} from "@/components/crm/DeleteCustomFieldMergeDialog";
...
export default function LeadCreatePanel({
  ...
  const title = entity === "contact" ? "Create New Contact" : "Create new lead";
  const subtitle =
    entity === "contact"
      ? "All standard and custom fields — use the gear to choose what appears"
      : "Add contact details, company, and pipeline. Use Fields to show or hide properties.";
  const hint =
    entity === "contact"
      ? "Add at least one of email, phone (mobile or alternate), or LinkedIn — any one is enough to create the contact."
      : "Include email, phone, or LinkedIn when you have them — or paste a job/freelance listing URL (https) from Upwork, Naukri, LinkedIn Jobs, etc.";
  const submitLabel = entity === "contact" ? "Create Contact" : "Create";
  ...
  const panelInner = (
    <>
      <CrmSlidePanelShell
        ...
        headerActions={
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-white px-3 py-1.5 text-sm font-semibold text-[var(--text-main)] hover:bg-[var(--background)] transition-colors"
          >
      ...
      >
        <form id={formId} ref={formRef} onSubmit={handleSubmit} className="space-y-5">
          <p className="text-sm text-[var(--text-muted)] leading-relaxed">{hint}</p>
          {entity === "contact" ? (
