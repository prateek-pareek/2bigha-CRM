/**
 * Suite-wide auth helpers (roles, stored user, permission checks).
 * Product folders must import from here — not from `@/lib/hrms/auth`
 * (`@/lib/hrms/auth` is only a temporary re-export).
 */
...
/**
 * True org admins that bypass module permission checks (Import / Export / Delete, etc.).
 * Sub Admin, Manager, Executive, and similar roles must use explicit grants from
 * Portal access & permissions — they are NOT unrestricted.
 */
export const isUnrestrictedAdmin = (user: User | null | undefined): boolean => {
    ...
    const compactRole = String(role || '')
        ...
        .toUpperCase()
        .replace(/[\s\-_]+/g, '');
    return fullAdminRoles.includes(normalizedRole) || fullAdminRoles.includes(compactRole);
};
...
/** @deprecated Prefer canViewEmployeeSalary — kept for callers during migration. */
export const canViewPayrollSensitive = (user: User | null): boolean => {
  return canViewEmployeeSalary(user);
};

/**
 * Who may see salary / stipend on employee profiles.
 * Requires `employees-salary:*` or payroll admin (not `payroll-self` alone).
 */
export const canViewEmployeeSalary = (user: User | null): boolean => {
  if (!user) return false;
  ...
  if (
    perms.some(
      (p) => p === "employees-salary" || p.startsWith("employees-salary:"),
    )
  ) {
    return true;
  }
...
/** Company payroll admin (run cycle, all slips, structures) — not self-service. */
export const canAccessPayrollAdmin = (user: User | null): boolean => {
  if (!user) return false;
  if (isPlatformBackupAdmin(user)) return true;
  const perms = (user.permissions || []).map((p) =>
    String(p || "").trim().toLowerCase(),
...
/** Own payslips / advances (or any payroll admin). */
export const canAccessPayrollSelf = (user: User | null): boolean => {
  if (!user) return false;
  if (canAccessPayrollAdmin(user)) return true;
  const perms = (user.permissions || []).map((p) =>
    String(p || "").trim().toLowerCase(),
  );
  return (
