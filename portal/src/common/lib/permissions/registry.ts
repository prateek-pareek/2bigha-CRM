...
> = {
  ...
  'pm-leads': [
    {
      perm: 'pm-leads:move_pipeline',
      label: 'Move PM leads between pipelines (board + bulk)',
    },
...
const CRM_MODULE_ROWS: Omit<PermissionModuleDef, 'suite'>[] = [
  ...
  // Role-based dashboards — Admin sees Admin only; Team Lead sees Team + Agent
  // (Agent page filters by team member); Agent sees Agent only.
  // Grant `workspace-admin:read` / `workspace-team:read` / `workspace-agent:read` per role.
  // `dashboard:read` alone (no tier key) maps to Admin.
  ...
  { id: 'reports-forecast', label: 'Reports · Pipeline Insights', routes: ['/crm/reports/forecast'] },
  ...
  { id: 'pm-leads', label: 'PM Leads', routes: ['/crm/pm/leads'] },
  { id: 'legal', label: 'Legal', routes: ['/crm/legal'] },
  { id: 'clients', label: 'Clients', routes: ['/crm/clients'] },
  ...
  { id: 'forms', label: 'Forms', routes: ['/crm/settings/forms'] },
