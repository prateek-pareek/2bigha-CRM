/**
 * HRMS app shell — layout from HRDashboard Figma kit, UI chrome from Dreams CRMS
 * (same language as our CRM).
 *
 * Layout ref: https://www.figma.com/design/sBG5ZDVaeJ2XrrDRnTbHMt/HRDashboard---HR-Management-Dashboard-UI-Kit--Community-
 * UI ref: https://crms.dreamstechnologies.com/html/leads.html
 */

import { suiteSidebarChrome } from "@/components/suite/shell/sidebar-chrome";

/** Root class for `/hrms` layout */
export const hrmsSuiteShellClassName =
  "theme-hrms hrms-app-root min-h-screen bg-[var(--background)] text-[var(--text-main)]";

/** Main content area — CRMS `.content` uses 20px padding on all sides */
export const hrmsShellLayout = {
  shell: "hrms-suite-shell flex h-full min-h-0 overflow-hidden bg-[var(--background)]",
  main: "hrms-app-main flex-1 overflow-y-auto bg-[var(--background)]",
  mainPadded: "p-5",
} as const;

export const HRMS_APP_ATTR = {
  root: "data-hrms-app",
  theme: "data-hrms-theme",
  themeValue: "crms",
} as const;

/**
 * Top app header — CRMS white bar (shared language with CRM).
 * Used by ModuleJiraAppHeader when productHref starts with `/hrms`.
 */
export const hrmsAppChrome = {
  header:
    "hrms-app-header theme-hrms z-50 flex h-[57px] shrink-0 items-center border-b border-[var(--border-color)] bg-white px-3 md:px-4",
  ...
  search:
    "hrms-header-search me-auto hidden min-w-0 max-w-[280px] flex-1 lg:block xl:max-w-[320px]",
  actions: "flex shrink-0 items-center gap-2",
  iconBtn:
    "hrms-header-icon inline-flex h-[38px] w-[38px] items-center justify-center rounded-[5px] border border-[#e8e8e8] bg-transparent text-[var(--text-main)] hover:bg-[var(--background)]",
  ...
  profileName: "sr-only",
  profileMenu:
    "invisible absolute right-0 top-full z-50 mt-1 w-44 rounded-[var(--radius-md)] border border-[var(--border-color)] bg-white py-1 opacity-0 shadow-[var(--hrms-shadow-raised)] transition-all group-hover/profile:visible group-hover/profile:opacity-100",
  workspaceMenu:
    "suite-workspace-menu z-50 w-[min(17.5rem,calc(100vw-4rem))] rounded-[var(--radius-md)] border border-[var(--border-color)] bg-white p-1 shadow-[var(--hrms-shadow-raised)]",
  workspaceMenuLabel:
...
/**
 * Sidebar — global suite rail (soft pill + icon tiles), shared with CRM.
 */
export const hrmsSidebarChrome = {
  ...suiteSidebarChrome,
  rail: "suite-jira-sidebar hrms-suite-sidebar-rail suite-sidebar-rail",
} as const;
