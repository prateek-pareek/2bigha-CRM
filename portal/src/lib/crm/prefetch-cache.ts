...
import { isAdmin } from "@/lib/hrms/auth";
...
export const crmCacheKeys = {
  ...
  workspace: (owner: string, window: string, sections: string) =>
    `workspace:${owner}:${window}:${sections}`,
  leadsList: (
    pipelineId: string,
  ...
  contactsList: (opts?: {
    ...
    `contacts:p=${opts?.page ?? 1}|ps=${opts?.pageSize ?? 25}|q=${opts?.search ?? ""}|sb=${opts?.sortBy ?? "createdAt"}|so=${opts?.sortOrder ?? "desc"}`,
  dealsList: (
    pipelineId: string,
