...
export interface LeadsPageProps {
  fixedVertical?: 'property_listing' | 'property_management';
  pageTitle?: string;
}

export default function CrmLeadsView({ fixedVertical, pageTitle }: LeadsPageProps = {}) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(effectiveViewModeKey, viewMode);
    }
  }, [viewMode, effectiveViewModeKey]);
  ...
  /** Active lead vertical (defaults to fixedVertical if provided). */
  const [activeLeadVertical, setActiveLeadVertical] = useState<
    '' | 'property_listing' | 'property_management'
  >(() => {
    if (fixedVertical) return fixedVertical;
    if (typeof window === 'undefined') return 'property_listing';
    const saved = localStorage.getItem(VERTICAL_STORAGE_KEY);
    return saved === 'property_listing' || saved === 'property_management' ? saved : 'property_listing';
  });
  useEffect(() => {
    if (typeof window !== 'undefined' && !fixedVertical) {
      localStorage.setItem(VERTICAL_STORAGE_KEY, activeLeadVertical);
    }
  }, [activeLeadVertical, fixedVertical]);
  ...
  const canMoveLeadsAcrossPipelines =
    ...
    (isPm && hasAccess('pm-leads:move_pipeline' as any));
  ...
  return (
    <div className={cn(CRM_LIST_PAGE, "crm-list-dense")}>
      <div className="crm-list-content">
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-0 pt-0 pb-0 w-full">
          <CrmPageHeader
            ...
            title={pageTitle || (isPm ? 'Property Management Leads' : 'Leads')}
            badge={<CrmCountBadge>{displayedTotal}</CrmCountBadge>}
            breadcrumbs={[
              { label: 'Home', href: '/crm/workspace/summary' },
              { label: pageTitle || (isPm ? 'Property Management Leads' : 'Leads') },
            ]}
            middle={
      ...
      <LeadCreatePanel
        ...
        initialLeadVertical={activeLeadVertical || (fixedVertical ?? 'property_listing')}
        lockVertical={!!fixedVertical}
        onSuccess={() => {
