...
export default function ConnectEmailAccountModal({ isOpen, onClose, onSuccess }: ConnectEmailAccountModalProps) {
  ...
  return (
    <div className={`${crmModalChrome.overlay} z-[1000] flex items-center justify-center p-4`}>
      ...
      <div className={`${crmModalChrome.centerShell} max-w-lg max-h-[90vh] crm-modal flex flex-col overflow-y-auto`}>
        ...
        <form onSubmit={handleSubmit} className={`${crmModalChrome.centerBody} space-y-6`}>
          <div>
            ...
            <select
              ...
              className="mt-2 block w-full rounded-[var(--radius-md)] border border-border bg-surface-dim/30 text-sm font-bold h-12 px-5 focus:ring-4 focus:ring-primary/5 outline-none"
            >
              {PROVIDERS.map((p) => (
          ...
          {!isOther && imapHost && smtpHost ? (
            <div className="rounded-[var(--radius-md)] border border-border bg-surface-dim/30 p-4 space-y-3">
          ...
          {(isGmail || isOutlook) && (
            <div className="space-y-3">
              <CrmButton
                type="button"
                variant="primary"
                className="w-full py-6 rounded-[var(--radius-md)] gap-2 justify-center text-sm font-bold"
              ...
              {isOutlook && (
                <p className="text-xs text-text-muted font-medium leading-relaxed rounded-[var(--radius-md)] border border-border bg-surface-dim/30 px-3 py-2.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="mt-2 block w-full rounded-[var(--radius-md)] border border-border bg-surface-dim/30 text-sm font-bold h-12 px-5"
                required
              />
            </div>
            <div>
          ...
          {isOther && (
            <div className="space-y-4 p-4 rounded-[var(--radius-md)] bg-surface-dim/30 border border-border">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 block w-full rounded-[var(--radius-md)] border border-border bg-card text-xs h-10 px-3"
                  />
                </div>
                <div>
          ...
          <div className="flex gap-4 pt-4">
            <CrmButton type="button" variant="secondary" onClick={onClose} className="flex-1 py-6 rounded-[var(--radius-md)]">
              Cancel
            </CrmButton>
            <CrmButton type="submit" variant="primary" disabled={submitting || !!oauthLoading} className="flex-[1.5] py-6 rounded-[var(--radius-md)] gap-2">
