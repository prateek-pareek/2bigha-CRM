...
function SortableField({ field, onDelete, onEdit }: { field: any, onDelete: (id: string) => void, onEdit: (field: any) => void }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex items-center justify-between p-4 mb-2 bg-card border rounded-[var(--radius-md)] transition-all ${isDragging ? 'border-primary shadow-xl scale-[1.02]' : 'border-[var(--border-color)] hover:border-primary/30'}`}
    >
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onDelete(field._id)} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-[var(--radius-md)] transition-colors">
          <Trash2 size={16} />
        </button>
      </div>
    </div>
...
export default function ManagePropertiesModal({ isOpen, onClose, module, onSuccess }: ManagePropertiesModalProps) {
  ...
  const content = (
    <div className={crmModalChrome.overlay}>
      ...
      <div className={`${crmModalChrome.slidePanel} max-w-md crm-modal`}>
        <div className={crmModalChrome.slideHeader}>
          <div className="min-w-0 flex-1">
            <h2 className={`${crmModalChrome.slideTitle} flex items-center gap-2`}>
              <Settings2 className="text-[var(--primary)]" size={18} strokeWidth={1.75} />
              Manage properties
            </h2>
        ...
        <div className={`${crmModalChrome.slideBody} bg-[var(--surface-dim)]`}>
          {loading ? (
             ...
             <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
               <SortableContext items={fields.map(f => f._id)} strategy={verticalListSortingStrategy}>
                 ...
                 {fields.length === 0 && (
                   <div className="text-center py-16 border-2 border-dashed border-[var(--border-color)] rounded-[var(--radius-md)]">
        ...
        <div className={`${crmModalChrome.slideFooter} flex-col items-stretch gap-3`}>
           <div className="flex items-center justify-center gap-2 rounded-[var(--radius-md)] border border-[var(--border-color)] bg-[var(--primary-light)] px-3 py-2 text-xs font-medium text-[var(--primary)]">
             <AlertCircle size={14} strokeWidth={1.75} /> Drag properties to reorder configuration
           </div>
           <button type="button" onClick={() => setIsCreateModalOpen(true)} className="inline-flex h-8 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] bg-[var(--primary)] px-4 text-sm font-medium text-white hover:bg-[var(--primary-dark)]">
