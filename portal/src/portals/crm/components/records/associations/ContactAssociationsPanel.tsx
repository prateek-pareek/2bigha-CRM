...
export default function ContactAssociationsPanel({
  contactId,
  contact,
  onUpdated,
  hideCompaniesSection = false,
}: {
  contactId: string;
  contact: Record<string, unknown>;
  onUpdated: () => void;
  /** When true, Companies are shown in CRMContactCompanySidebarCard instead */
  hideCompaniesSection?: boolean;
}) {
  ...
  const leads = useMemo(() => {
    const raw = (contact?.associatedLeads as AnyRec[]) || [];
    ...
    return [asLead, ...raw];
  }, [contact?.associatedLeads, contact?.sourceLead]);
  ...
  const idsFor = (field: string) =>
    ((contact[field] as AnyRec[]) || []).map((x) => String(x._id || x));

  const patchAssociations = async (body: Record<string, string[]>) => {
    const token = localStorage.getItem("token");
    const res = await fetch(`${CRM_API_URL}/crm/contacts/${contactId}`, {
      method: "PATCH",
  ...
  const remove = (field: string, id: string) => {
    if (field === "associatedLeads" && sourceLeadId && id === sourceLeadId) {
      toast.message("This lead is the source record for this contact and stays linked.");
      return;
    }
  ...
  const addExisting = (field: string, id: string) => {
    ...
    if (field === "associatedContacts" && id === contactId) {
      toast.error("Cannot associate a contact with itself");
      return;
    }
    ...
    <div className="bg-card border border-border rounded-[var(--crm-radius-ui)] p-6 shadow-sm">
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          {people.length === 0 ? (
            <p className="text-xs text-text-muted italic">No other contacts linked</p>
          ) : (
            <div className="space-y-2">
              {people.map((c) => (
                <Row
