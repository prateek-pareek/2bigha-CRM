...
import { mergeFilterValueOptions } from "@/lib/crm/filter-config";
...
interface FilterValueSelectorProps {
  ...
  operator: string;
  pipelineId?: string;
  /** Catalog / custom-field definition options — always shown, even if unused on records. */
  knownOptions?: string[];
}
...
export function FilterValueSelector({
  ...
  const [options, setOptions] = useState<string[]>(() =>
    mergeFilterValueOptions(knownOptions),
  );
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setOptions(mergeFilterValueOptions(knownOptions));
        }
      } finally {
  ...
  }, [fieldKey, module, pipelineId, knownOptions?.join("\0")]);
