...
export default function CRMSavedViews({
  ...
  return (
    <div className="relative shrink-0">
      ...
      >
        {loading ? null : activeView ? activeView.name : `All ${label}`}
      </CrmButton>

      {isOpen && (
        <>
          <div className="absolute top-full left-0 mt-2 z-[300] w-56 bg-card rounded-[var(--crm-radius-ui)] border border-[var(--border-color)] shadow-[var(--crm-shadow-raised)] overflow-hidden">
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-left text-sm font-medium transition-all ${
                !activeViewId
                  ...
                  : 'hover:bg-surface-dim text-text-main'
              }`}
            >
            ...
            {views.map(v => (
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 px-4 py-2.5 text-left text-sm font-medium transition-all hover:bg-surface-dim ${
                  activeViewId === v._id
                    ? 'bg-[var(--primary-light)] text-[var(--primary)]'
                    : 'text-text-main'
                }`}
              >
                ...
                {activeViewId === v._id && <Check size={16} strokeWidth={2} />}
              </button>
            ))}
      ...
      {isSaveOpen && (
        <div className={cn(crmModalChrome.overlay, 'z-[999] flex items-center justify-center p-4')}>
          ...
          <div className={cn(crmModalChrome.centerShell, 'max-w-sm crm-modal')}>
      ...
      {isManageOpen && (
        <div className={cn(crmModalChrome.overlay, 'z-[999] flex items-center justify-center p-4')}>
          ...
          <div className={cn(crmModalChrome.centerShell, 'max-w-md crm-modal flex flex-col')}>
            <div className={crmModalChrome.centerHeader}>
              <div>
            ...
            <div className={crmModalChrome.centerFooter}>
              <CrmButton className="w-full" onClick={() => setIsManageOpen(false)}>
                Close
              </CrmButton>
            </div>
          </div>
