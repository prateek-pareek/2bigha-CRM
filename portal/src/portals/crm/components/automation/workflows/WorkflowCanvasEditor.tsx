...
import { normalizeWorkflowFilter } from "@/lib/crm/automation/workflow-filter-utils";
...
export type { WorkflowCanvasEntityKind } from "@/lib/crm/automation/workflow-field-presets";
...
function StartNode() {
  return (
    <div className="rounded-[var(--radius-md)] border-2 border-violet-400 bg-gradient-to-br from-violet-50 to-white px-4 py-3 shadow-md min-w-[140px]">
...
function WfActionNode({
  ...
  return (
    <div
      className={`rounded-[var(--radius-md)] border bg-white px-3 py-2 shadow-md min-w-[220px] max-w-[320px] ${
        selected ? "border-violet-500 ring-2 ring-violet-200" : "border-[var(--border-color)]"
      }`}
    >
      ...
      <select
        className="mt-1 w-full rounded-lg border border-[var(--border-color)] py-1 px-1 text-xs font-bold"
        value={t}
      ...
      <div className="mt-1 space-y-0.5">
        ...
        {t === "create_task" && (
          <>
            <input
              className="w-full rounded-lg border border-[var(--border-color)] px-1 py-0.5 text-xs"
              placeholder="title"
            ...
            />
            <label className="inline-flex items-center gap-1 text-[10px] text-[var(--text-main)]">
              <input
                type="checkbox"
                checked={action.calendarEnabled !== false}
                onChange={(e) => patchAction({ calendarEnabled: e.target.checked })}
              />
              Calendar
            </label>
            <label className="inline-flex items-center gap-1 text-[10px] text-[var(--text-main)]">
              <input
                type="checkbox"
                checked={action.reminderEnabled !== false}
                onChange={(e) => patchAction({ reminderEnabled: e.target.checked })}
              />
              Reminder
            </label>
            {action.reminderEnabled !== false ? (
              <select
                className="w-full rounded-lg border border-[var(--border-color)] px-1 py-0.5 text-[10px]"
                value={Number(action.reminderBeforeMinutes ?? 0)}
                onChange={(e) =>
                  patchAction({ reminderBeforeMinutes: Number(e.target.value) })
                }
              >
        ...
        {t === "notify_teams" && (
          <>
            ...
            <input
              className="nodrag nopan w-full rounded-lg border border-[var(--border-color)] px-1 py-0.5 text-xs"
...
function WfDelayNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`rounded-[var(--radius-md)] border bg-amber-50 px-3 py-2 shadow-md min-w-[180px] ${
...
function WfConditionNode({
  ...
  return (
    <div
      className={`rounded-[var(--radius-md)] border bg-violet-50 px-3 py-2 shadow-md min-w-[260px] max-w-[320px] ${
...
function WfWaitEmailEngagementNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`rounded-[var(--radius-md)] border bg-sky-50 px-3 py-2 shadow-md min-w-[200px] max-w-[260px] ${
...
function WfAbSplitNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`rounded-[var(--radius-md)] border bg-fuchsia-50 px-3 py-2 shadow-md min-w-[200px] ${
...
export default function WorkflowCanvasEditor({
  ...
  return (
    <ReactFlowProvider>
      <div
        className={`${surfaceClass} w-full rounded-[var(--radius-md)] border border-[var(--border-color)]/90 bg-gradient-to-b from-slate-100/40 via-white to-slate-50/30 shadow-inner ring-1 ring-slate-100/80 overflow-hidden`}
      >
        <ReactFlow
          nodes={nodes}
          edges={edges}
        ...
        >
          ...
          <Controls className="!shadow-lg !border-[var(--border-color)]/80 !rounded-[var(--radius-md)] !overflow-hidden" />
          <MiniMap
            className="!rounded-[var(--radius-md)] !border !border-[var(--border-color)]/80 !shadow-md !bg-white/90"
            maskColor="rgb(148, 163, 184, 0.12)"
          />
          <Panel position="top-left" className="m-2 flex flex-wrap gap-1.5 rounded-[var(--radius-md)] border border-[var(--border-color)]/80 bg-white/95 p-1.5 shadow-lg">
            <button
              type="button"
