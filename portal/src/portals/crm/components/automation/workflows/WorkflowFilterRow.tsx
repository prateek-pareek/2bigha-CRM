  ...
  return (
    <div ref={ref} className={`relative ${className ?? ""}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between gap-1.5 border border-border/60 bg-surface-dim/30 rounded-[var(--radius-md)] px-3 py-2 text-sm font-medium text-text-main outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all cursor-pointer hover:border-primary/30 hover:bg-[var(--primary-light)] min-w-0"
      >
      ...
      {open && (
        <ul className="absolute z-50 mt-1 min-w-full w-max max-w-[260px] max-h-56 overflow-y-auto rounded-[var(--radius-md)] border border-border bg-card shadow-lg py-1">
          {placeholder && (
            <li>
              <button
                type="button"
                onClick={() => { onChange(""); setOpen(false); }}
                className={`w-full text-left px-3 py-1.5 text-sm font-medium transition-colors ${value === "" ? "bg-[var(--primary-light)] text-primary font-semibold" : "text-text-muted hover:bg-[var(--primary-light)] hover:text-primary"}`}
              >
                {placeholder}
              </button>
            </li>
          ...
          {options.map((opt) => (
            <li key={opt.value}>
              <button
                ...
                className={`w-full text-left px-3 py-1.5 text-sm font-medium transition-colors ${opt.value === value ? "bg-[var(--primary-light)] text-primary font-semibold" : "text-text-main hover:bg-[var(--primary-light)] hover:text-primary"}`}
              >
                {opt.label}
              </button>
            </li>
...
const FORM_INPUT =
  "border border-border/60 bg-surface-dim/30 rounded-[var(--radius-md)] px-3 py-2 text-sm font-medium text-text-main outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all placeholder:text-text-muted/60";
const GOAL_INPUT =
