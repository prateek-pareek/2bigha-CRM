...
export async function previewCrmSegmentCounts(body: {
  listType?: CrmSegmentListType;
  leadFilters?: FilterCriteria[];
  contactFilters?: FilterCriteria[];
  members?: Array<{ module: CrmSegmentMemberModule; entityId: string }>;
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err?.message || "Failed to preview segment counts");
  }
  return res.json();
}

export async function previewCrmSegmentMembers(
  body: {
    listType?: CrmSegmentListType;
    leadFilters?: FilterCriteria[];
    contactFilters?: FilterCriteria[];
  ...
  const res = await fetch(`${CRM_API_URL}/crm/segments/preview-members`, {
    method: "POST",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err?.message || "Failed to preview segment members");
  }
  return res.json();
}

export async function cloneCrmSegment(id: string): Promise<CrmSegment> {
  const res = await fetch(`${CRM_API_URL}/crm/segments/${encodeURIComponent(id)}/clone`, {
    method: "POST",
    headers: authHeaders(),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err?.message || "Failed to duplicate segment");
  }
  return res.json();
}

export type CrmSegmentAssignLeadsResult = {
  segmentId: string;
  ownerName: string;
  scope: "selected" | "all";
  requested: number;
...
export async function assignCrmSegmentLeads(
  id: string,
  body: {
    ownerName: string;
    leadIds?: string[];
...
): Promise<CrmSegmentAssignLeadsResult> {
  const res = await fetch(
    `${CRM_API_URL}/crm/segments/${encodeURIComponent(id)}/assign-leads`,
    {
      method: "POST",
      headers: { ...authHeaders(), "Content-Type": "application/json" },
      body: JSON.stringify(body),
