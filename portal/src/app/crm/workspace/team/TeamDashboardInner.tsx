...
import { canPickAnyDashboardEmployee } from "@/lib/crm/shared/dashboard-access";
...
import { Users, PhoneCall, PhoneOff, Home, CheckCircle2, ClipboardCheck, Trophy } from "lucide-react";
...
export default function TeamDashboardInner() {
  const { hasAccess, isLoaded } = usePermissions();
  const canPickTeamLead = canPickAnyDashboardEmployee(hasAccess);
  ...
  const [teamLeadOptions, setTeamLeadOptions] = useState<DashboardPersonOption[]>([]);
  const [selectedTeamLeadId, setSelectedTeamLeadId] = useState(
    () => searchParams.get("teamLead") || "",
  );

  const setTeamLead = useCallback(
    (id: string) => {
      setSelectedTeamLeadId(id);
      setSelectedMember(null);
      const params = new URLSearchParams(searchParams.toString());
      if (id) params.set("teamLead", id);
      else params.delete("teamLead");
  ...
  useEffect(() => {
    const fromUrl = searchParams.get("teamLead") || "";
    setSelectedTeamLeadId((prev) => (prev === fromUrl ? prev : fromUrl));
  }, [searchParams]);

  useEffect(() => {
    if (!isLoaded || !canPickTeamLead) return;
    let active = true;
    fetch(`${CRM_API_URL}/crm/dashboard/admin?window=${dateRange}`, { headers: authHeaders() })
      .then((r) => (r.ok ? r.json() : null))
      .then((res) => {
        if (!active || !res) return;
        const rows = Array.isArray(res.teamPerformance) ? res.teamPerformance : [];
        const opts: DashboardPersonOption[] = rows
          .filter((t: any) => t.teamId)
          .map((t: any) => ({
            id: String(t.teamId),
            label: t.teamLead || t.team || "Team Lead",
            sublabel: t.team && t.team !== t.teamLead ? t.team : undefined,
          }));
        setTeamLeadOptions(opts);
        if (!selectedTeamLeadId && opts.length === 1) {
          setTeamLead(opts[0].id);
        }
      })
      .catch(() => active && setTeamLeadOptions([]));
    return () => {
      active = false;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoaded, canPickTeamLead, dateRange]);

  const load = useCallback(() => {
    let active = true;
    setLoading(true);
    setError(null);
    if (canPickTeamLead && !selectedTeamLeadId) {
      setData(null);
      setLoading(false);
      return () => {
        active = false;
    ...
    if (canPickTeamLead && selectedTeamLeadId) {
      qs.set("teamLead", selectedTeamLeadId);
    }
    fetch(`${CRM_API_URL}/crm/dashboard/team?${qs.toString()}`, { headers: authHeaders() })
      .then((r) => {
  ...
  }, [dateRange, canPickTeamLead, selectedTeamLeadId]);
  ...
  useEffect(() => {
    ...
    setMemberDetail(null);
    fetch(
      `${CRM_API_URL}/crm/dashboard/agent?window=${dateRange}&agent=${selectedMember.id}`,
      { headers: authHeaders() },
    )
      .then((r) => (r.ok ? r.json() : null))
  ...
  const selectedLeadLabel = useMemo(() => {
    if (!selectedTeamLeadId) return null;
    return teamLeadOptions.find((o) => o.id === selectedTeamLeadId)?.label || null;
  }, [selectedTeamLeadId, teamLeadOptions]);
  ...
  const waitingForAdminPick = canPickTeamLead && !selectedTeamLeadId;
  ...
  return (
    <DashboardShell
      ...
      description={
        canPickTeamLead
          ? "Pick a team lead to view that team's performance, members and targets."
          : "Your team's performance, members and targets — no other team's data."
      }
      dateRange={dateRange}
      ...
      extraFilters={
        canPickTeamLead ? (
          <DashboardPersonSelect
            ...
            placeholder={teamLeadOptions.length ? "Select team lead…" : "Loading…"}
          />
        ) : undefined
      }
    >
      {waitingForAdminPick ? (
        <div className="rounded-lg border border-dashed border-border bg-muted/20 px-6 py-16 text-center">
          ...
          <p className="text-sm font-medium text-card-foreground">Select a team lead</p>
          <p className="mt-1 text-sm text-muted-foreground">
            Choose a team lead above to load their team dashboard.
          </p>
        </div>
      ) : error ? (
        ...
        <div className="space-y-6">
          <Panel
            title="Team Performance — This Period"
            subtitle={`${data?.teamSize || 0} team members${selectedLeadLabel ? ` · ${selectedLeadLabel}` : ""}`}
          >
